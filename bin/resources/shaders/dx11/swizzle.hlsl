...
RWTexture2D<float>        DepthTexture : register(u3);
RWTexture2D<uint>         RawTexture   : register(u4);
...
uint depth_to_word(float depth)
{
  return uint(depth * exp2(32.0f));
}

uint depth_to_short(float depth)
{
  return uint(depth * exp2(32.0f)) & 0xFFFF;
}
...
float word_to_depth(uint word)
{
  return float(word) * exp2(-32.0f);
}

float short_to_depth(uint short)
{
  return float(short & 0xFFFF) * exp2(-32.0f);
}
...
void main(uint3 LocalThreadId : SV_GroupThreadID, uint3 WorkGroupId : SV_GroupID)
{
  ...
  uint base_word_addr = (BP * 64 + page * 2048) % (1024 * 1024); // First page
  ...
  switch (PSM)
  {
    ...
    case PSMZ24:
    {
      uint word_addr = xor_z32(base_word_addr + word_addr_c32(mem_xy));
      if (type == TYPE_TEXTURE_TO_MEM)
      {
        float depth = DepthTexture[tex_xy];
        uint word = depth_to_word(depth);
        if (PSM == PSMZ24)
          StoreWord24(word_addr, word);
        else
          StoreWord(word_addr, word);
      ...
      else if (type == TYPE_MEM_TO_TEXTURE)
      {
        ...
        if (PSM == PSMZ24)
          word &= 0xFFFFFFu;
        float depth = word_to_depth(word);
        DepthTexture[tex_xy] = depth;
      }
      else if (type == TYPE_RAW_TO_MEM)
      {
        uint word = RawTexture[tex_xy];
    ...
    case PSMZ16S:
    {
      ...
      {
        float depth = DepthTexture[tex_xy];
        uint shrt = depth_to_short(depth);
        StoreShort(short_addr, shrt);
      }
      else if (type == TYPE_MEM_TO_TEXTURE)
      {
        uint shrt = LoadShort(short_addr);
        float depth = short_to_depth(shrt);
        DepthTexture[tex_xy] = depth;
      }
      else if (type == TYPE_RAW_TO_MEM)
      {
