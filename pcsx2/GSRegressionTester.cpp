...
void SpinlockSharedMemory::LockWrite()
{
	while (true)
	{
...
void SpinlockSharedMemory::LockRead()
{
	while (true)
	{
		if (InterlockedCompareExchange(&lock, READABLE, READABLE) == 0)
		{
			return;
		}
...
void SpinlockSharedMemory::UnlockWrite()
{
	InterlockedExchange(&lock, READABLE);
}


void SpinlockSharedMemory::UnlockRead()
{
	InterlockedExchange(&lock, WRITEABLE);
}
...
{	bool expected = false;
	while (lock.compare_exchange_strong(expected, true, std::memory_order_acquire, std::memory_order_relaxed))
	{		std::this_thread::yield();
	}
...
Mutex::Mutex()
{	
	handle = CreateMutex(nullptr, FALSE, L"Mutex");
}
...
#ifdef __WIN32__
struct SpinLocalSharedMemory
{
	volatile LONG lock = 0; // must be in shared memory
	...
	void Lock()
	{
		...
		{
			LONG expected = 0;
			if (InterlockedCompareExchange(&lock, 1, expected) == 0)
			{
				return;
			}
