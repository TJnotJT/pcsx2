#include "GS/Renderers/HW/GSRendererHWHybrid.h"
#include "GS/GSGL.h"
...
GSRendererHWHybrid::GSRendererHWHybrid()
	: m_texture_cache(std::make_unique<GSTextureCacheHybrid>())
{
}
...
GSTexture* GSRendererHWHybrid::GetOutput(int i, float& scale, int& y_offset)
{
	return nullptr;
}

void GSRendererHWHybrid::InvalidateVideoMem(const GIFRegBITBLTBUF& BITBLTBUF, const GSVector4i& r)
{
}

void GSRendererHWHybrid::InvalidateLocalMem(const GIFRegBITBLTBUF& BITBLTBUF, const GSVector4i& r, bool clut)
{
}

void GSRendererHWHybrid::ConvertVertices(GSVertex* input, u32 count, std::vector<GSHybridVertex>& output)
{
...
GSHybridDrawMesh GSRendererHWHybrid::GetDrawMesh()
{
...
GSRendererHWHybrid::CachedDrawTextures GSRendererHWHybrid::LookupCachedDrawTextures()
{
	...
	// Frame
	{
		GSTextureCacheHybrid::GSMemoryRect rect(frame, 512);
	...
	if (zbuf.ZMSK == 0)
	{
		GSTextureCacheHybrid::GSMemoryRect rect(zbuf, frame, 512);
...
GSHybridDrawTextures GSRendererHWHybrid::GetBackendDrawTextures(const CachedDrawTextures& cached)
{
...
u32 GSRendererHWHybrid::GetBackendTopology()
{
...
GSVector4i GSRendererHWHybrid::GetViewport(const GSHybridDrawTextures& textures)
{
	if (textures.rt)
...
GSHybridDrawSelector GSRendererHWHybrid::GetSelector(const CachedDrawTextures& textures)
{
...
GSHybridConstantBuffer GSRendererHWHybrid::GetConstantBuffer(const CachedDrawTextures& textures)
{
...
bool GSRendererHWHybrid::IsCoverageAlphaSupported()
{
	return false;
}
