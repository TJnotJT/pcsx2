	...
	, m_state(TextureState::Clean)
{
	...
	if (GSLocalMemory::m_psm[rect.PSM].depth)
		m_texture.reset(g_gs_device->CreateDepthStencil(size, false));
	else
		m_texture.reset(g_gs_device->CreateRenderTarget(size, GSTexture::Format::Color, false));

	// Create a CLUT if the PSM requires one.
	if (need_clut)
	{
		m_clut.reset(g_gs_device->CreateRenderTarget(GSVector2i(256, 1), GSTexture::Format::Color, false));
