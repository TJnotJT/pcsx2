	...
	bool is_shuffle, bool possible_clear, bool preserve_scale, GSTextureCache::Source* src, GSTextureCache::Target* ds, int offset)
{
	...
	else if (!GSConfig.UserHacks_DisableDepthSupport)
	{
		...
		if (dst_match)
		{
			...
			const bool dst_match_int = dst_match->m_texture->IsDepthInteger(); // source
			const bool dst_int = dst->m_texture->IsDepthInteger(); // destination
	...
	GSTextureCache::Source* src, GSTextureCache::Target* ds, int offset)
{
	...
	if (dst->m_scale != rescaler.m_scale && (!preserve_scale || (type == RenderTarget && (is_shuffle || !dst->m_downscaled || TEX0.TBW != dst->m_TEX0.TBW))))
	{
		...
		else
		{
			...
			g_gs_device->StretchRect(dst->m_texture, FullSrcRect, tex, rescaler.m_dRect, shader, type == RenderTarget && !preserve_scale);
		}
	...
	if (type == DepthStencil && dst->m_type == DepthStencil && GSLocalMemory::m_psm[dst->m_TEX0.PSM].trbpp == 32 && GSLocalMemory::m_psm[TEX0.PSM].trbpp == 24 && dst->m_alpha_max > 0 && !dst->m_texture->IsDepthInteger())
	{
		...
		if (!tex)
			return nullptr;
		g_gs_device->StretchRect(dst->m_texture, FullSrcRect, tex, rescaler.m_dRect, GetDepth32to24Shader(dst->m_texture->IsDepthInteger(), tex->IsDepthInteger()), false);
		g_gs_device->Recycle(dst->m_texture);
	...
	else if ((used || type == GSTextureCache::DepthStencil) && (std::abs(static_cast<s16>(GSLocalMemory::m_psm[dst->m_TEX0.PSM].bpp - GSLocalMemory::m_psm[TEX0.PSM].bpp)) == 16))
	{
		...
		if (!is_shuffle)
		{
			...
			if (src && src->m_from_target && src->m_from_target == dst)
			{
				...
				{
					GSTexture* tex = g_gs_device->CreateCompatibleTexture(dst->m_texture, rescaler.m_new_scaled_size.x, rescaler.m_new_scaled_size.y, rescaler.m_clear);
					if (!tex)
						return nullptr;
			...
			else
			{
				...
				else
				{
					...
					const ShaderConvert shader = GetCopyShader(type, dst->m_texture->IsDepthInteger(), tex->IsDepthInteger());
					g_gs_device->StretchRect(dst->m_texture, source_rect, tex, rescaler.m_dRect, shader, false);
		...
		if (!is_frame)
		{
			...
			for (Target* dst_match : m_dst[1 - type])
			{
				...
				else
				{
					...
					const ShaderConvert shader = GetColorDepthShader(true, GSLocalMemory::m_psm[dst->m_TEX0.PSM].trbpp, false, dst->m_texture->IsDepthInteger());
				...
				if (t != dst && t->Overlaps(dst->m_TEX0.TBP0, dst->m_TEX0.TBW, dst->m_TEX0.PSM, dst_valid) && GSUtil::HasSharedBits(dst->m_TEX0.PSM, t->m_TEX0.PSM))
				{
						...
						((((dst->m_TEX0.TBP0 - t->m_TEX0.TBP0) >> 5) % std::max(t->m_TEX0.TBW, 1U)) + (dst_valid.z / 64)) <= dst->m_TEX0.TBW)
					{
							...
							GSLocalMemory::m_psm[t->m_TEX0.PSM].bpp == GSLocalMemory::m_psm[dst->m_TEX0.PSM].bpp)
						{
							...
							const bool dst_int = dst->m_texture->IsDepthInteger();
							const u32 bpp = GSLocalMemory::m_psm[dst->m_TEX0.PSM].trbpp;
							const ShaderConvert shader = GetColorDepthShader(true, bpp, false, dst_int);
...
bool GSTextureCache::CopyRGBFromDepthToColor(Target* dst, Target* depth_src)
{
	...
	const bool depth_int = depth_src->m_texture->IsDepthInteger();
...
bool GSTextureCache::Move(u32 SBP, u32 SBW, u32 SPSM, int sx, int sy, u32 DBP, u32 DBW, u32 DPSM, int dx, int dy, int w, int h)
{
	...
	if (SBP == DBP && (!(GSVector4i(sx, sy, sx + w, sy + h).rintersect(GSVector4i(dx, dy, dx + w, dy + h))).rempty() || renderer_is_directx))
	{
		...
		if (tmp_texture->IsDepthStencilOrDepthInteger())
		{
	...
	else
	{
		...
		else if (src->m_type != dst->m_type)
		{
			...
			const bool dst_int = dst->m_texture->IsDepthInteger();
			ShaderConvert shader = GetColorDepthShader(dst->m_type == DepthStencil, dpsm_s.trbpp, src_int, dst_int);
			g_gs_device->StretchRect(src->m_texture, src_rect, dst->m_texture, scaled_src_rect, shader, false);
		}
		else if (src->m_texture->IsDepthStencilOrDepthInteger())
		{
			const GSVector4 dst_rect = GSVector4(scaled_dx, scaled_dy, (scaled_dx + scaled_w), (scaled_dy + scaled_h));
			g_gs_device->StretchRect(src->m_texture, src_rect, dst->m_texture, dst_rect,
				GetDepthCopyShader(src->m_texture->IsDepthInteger(), dst->m_texture->IsDepthInteger()), false);
		}
		else
		{
...
GSTextureCache::Source* GSTextureCache::CreateSource(const GIFRegTEX0& TEX0, const GIFRegTEXA& TEXA, const GIFRegCLAMP& CLAMP, Target* dst, int x_offset, int y_offset, const GSVector2i* lod, const GSVector4i* src_range, GSTexture* gpu_clut, SourceRegion region, bool force_temp)
{
	...
	else if (dst)
	{
		...
		ShaderConvert shader = dst->m_type != RenderTarget ?
			(dst->m_texture->IsDepthInteger() ? ShaderConvert::UINT32_TO_RGBA8 : ShaderConvert::FLOAT32_TO_RGBA8) :
			ShaderConvert::COPY;
	...
	if (m_type == DepthStencil && g_texture_cache->GetTemporaryZ() != nullptr)
	{
		if (g_texture_cache->GetTemporaryZInfo().ZBP == m_TEX0.TBP0)
		{
			...
			if (m_TEX0.TBP0 == z_address_info.ZBP)
			{
				...
				GSTexture* temp_z = g_texture_cache->GetTemporaryZ();
				const ShaderConvert shader = GetDepthCopyShader(m_texture->IsDepthInteger(), temp_z->IsDepthInteger());
				const GSVector4 srect = GSVector4(
					total_rect.x / static_cast<float>(m_unscaled_size.x),
				...
				g_gs_device->StretchRect(m_texture, srect, temp_z, GSVector4(dRect), shader, false);
			}
		}
	}
...
bool GSTextureCache::Target::ResizeTexture(int new_unscaled_width, int new_unscaled_height, bool recycle_old, bool require_new_rect, GSVector4i new_rect, bool keep_old)
{
	...
	if (m_texture->GetState() == GSTexture::State::Dirty)
	{
		...
		if (tex->IsDepthStencilOrDepthInteger())
		{
			...
			g_gs_device->StretchRect(m_texture, tex, GSVector4(rc), GetDepthCopyShader(m_texture->IsDepthInteger(), tex->IsDepthInteger()), false);
		}
		else
		{
			if (require_new_rect)
