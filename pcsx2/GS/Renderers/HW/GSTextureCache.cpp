	...
	if (dst && (x_offset != 0 || y_offset != 0) && (TEX0.PSM != PSMT8 || channel_shuffle))
	{
		...
		if constexpr (force_target_copy)
		{
			...
			GSTexture* dTex = outside_target ?
				g_gs_device->FetchSurface(dst->m_texture->GetUsage(), w, h, 1, dst->m_texture->GetFormat(), true, PreferReusedLabelledTexture()) :
	...
	else if (dst)
	{
		...
		else
		{
			...
			GSTexture* dTex = use_texture ?
				g_gs_device->CreateTexture(new_size, 1, dst->m_texture->GetFormat(), PreferReusedLabelledTexture()) :
				g_gs_device->FetchSurface(dst->m_texture->GetUsage(), new_size, 1, dst->m_texture->GetFormat(),
				...
				if (is_8bits)
				{
					...
					if (dst->GetScale() > 1.0f)
					{
						GSTexture* tmpTex = use_texture ?
							g_gs_device->CreateTexture(dst->m_unscaled_size, 1, dst->m_texture->GetFormat(), PreferReusedLabelledTexture()) :
							g_gs_device->FetchSurface(dst->m_texture->GetUsage(), dst->m_unscaled_size, 1, dst->m_texture->GetFormat(), false, 
...
GSTextureCache::Target* GSTextureCache::Target::Create(GIFRegTEX0 TEX0, int w, int h, float scale, int type, bool clear)
{
	...
	GSTexture* texture = (type == RenderTarget) ?
		g_gs_device->CreateFeedbackTarget(scaled_w, scaled_h, GSTexture::Format::Color, clear, PreferReusedLabelledTexture()) :
		(g_gs_device->Features().depth_feedback ?
			g_gs_device->CreateFeedbackDepth(scaled_w, scaled_h, clear, PreferReusedLabelledTexture()) :
