...
void GSRendererHW::SetupSpriteRoundClampAlign(GSTextureCache::Target* rt, GSTextureCache::Target* ds, GSTextureCache::Source* tex)
{
	...
	if (GetVertexUVRoundingInfo(tex_enabled, target->GetScale() != 1.0, tex_enabled ? &info : nullptr))
	{
		...
		if (tex_enabled)
		{
			...
			{
				// Shift to remove the native half texel and add an upscaled half texel.
				const float shift = 8.0f / tex_scale - 8.0f;

				// Note, below that the texture offset is subtracted from the UV coords in the shader
				// so we need to modify it the opposite of what we want here.
				if (info.half_offset_XU)
				{
					if (info.same_dir_XU)
					{
						GL_INS("HW: Detected XU bilinear blur, adding %.4f texels to U.", -shift / 16.0f);
						m_conf.cb_vs.texture_offset.x += shift;
					}
				...
				if (info.half_offset_YV)
				{
					if (info.same_dir_YV)
					{
						GL_INS("HW: Detected YV bilinear blur, adding %.4f texels to V.", -shift / 16.0f);
						m_conf.cb_vs.texture_offset.y += shift;
					}
