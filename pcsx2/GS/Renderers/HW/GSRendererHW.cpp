...
void GSRendererHW::EmulateZbuffer(const GSTextureCache::Target* ds)
{
	...
	// We only need to fix Z values if the value if written to the Z buffer or
	// if we are depth testing and using ZTST_GREATER. The latter requirement is because
	// both clamp and floor only decrease Z, so only ZTST_GREATER can be broken by failing
	// to clamp/floor the incoming Z.
