...
void GSRendererHW::DetermineROVUsage(GSTextureCache::Target* rt, GSTextureCache::Target* ds)
{
	...
	const bool color_is_rov = rt && rt->m_texture->IsShaderReadWriteMode();
	const bool depth_is_rov = ds && ds->m_texture->IsShaderReadWrite();
...
void GSRendererHW::ConvertTextureTypeROVSingle(GSTextureCache::Target* tgt, bool shader_write)
{
	...
	GSTexture::ShaderAccess access = shader_write ? GSTexture::ShaderAccess::ReadWrite : GSTexture::ShaderAccess::ReadOnly;
	GSTexture* new_tex = depth ?
		(shader_write ?
			g_gs_device->CreateRenderTarget(old_tex->GetSize(), GSTexture::Format::DepthColor, false, true, access) :
			g_gs_device->CreateDepthStencil(old_tex->GetSize(), false, true)) :
		g_gs_device->CreateRenderTarget(old_tex->GetSize(), GSTexture::Format::Color, false, true, access);
...
void GSRendererHW::ConvertTextureTypeROV(GSTextureCache::Target* rt, GSTextureCache::Target* ds)
{
	...
	if (ds)
	{
		if (m_conf.ps.HasDepthROV() && !ds->m_texture->IsShaderReadWrite())
		{
	...
	if (rt && m_conf.ps.HasColorROV() && !rt->m_texture->IsShaderReadWrite())
	{
