...
void GSRendererHW::EmulateDither()
{
	if (m_conf.ps.dither || m_conf.blend_multi_pass.dither)
	{
		...
		GL_DBG("DITHERING mode %s (%s)", (GSConfig.Dithering == 3) ? "Force 32bit" : ((GSConfig.Dithering == 0) ? "Disabled" : "Enabled"), GSConfig.Dithering);
...
void GSRendererHW::DetermineROVUsage(GSTextureCache::Target* rt, GSTextureCache::Target* ds)
{
	...
	const bool color_is_rov = rt && rt->m_texture->IsRWMode();
	const bool depth_is_rov = ds && ds->m_texture->IsRWRenderTarget();
...
void GSRendererHW::ConvertTextureTypeROVSingle(GSTextureCache::Target* tgt, bool rw_rt)
{
	...
	GSTexture* new_tex = depth ?
		(rw_rt ? g_gs_device->CreateRWRenderTarget(old_tex->GetSize(), GSTexture::Format::DepthColor, false, true) :
			g_gs_device->CreateDepthStencil(old_tex->GetSize(), false, true)) :
		(rw_rt ? g_gs_device->CreateRWRenderTarget(old_tex->GetSize(), GSTexture::Format::Color, false, true) :
			g_gs_device->CreateRenderTarget(old_tex->GetSize(), GSTexture::Format::Color, false, true));
...
void GSRendererHW::ConvertTextureTypeROV(GSTextureCache::Target* rt, GSTextureCache::Target* ds)
{
	...
	if (ds)
	{
		if (m_conf.ps.HasDepthROV() && !ds->m_texture->IsRWRenderTarget())
		{
	...
	// Convert color to the proper type. This only converts from RenderTarget to RWRenderTarget,
	// and not the other way, since a color RWRenderTarget can do everything a color RenderTarget can.
	if (rt && m_conf.ps.HasColorROV() && !rt->m_texture->IsRWRenderTarget())
	{
		GL_PUSH("HW: Convert RenderTarget -> RWRenderTarget for ROV.");
