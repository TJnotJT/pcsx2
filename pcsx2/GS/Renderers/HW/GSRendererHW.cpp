...
void GSRendererHW::ConfigureROV(bool rov_color, bool rov_depth, bool oneshot)
{
	...
	if (rov_depth)
	{
		...
		if (oneshot)
		{
			...
			m_conf.rov_depth = GSHWDrawConfig::ONESHOT_ROV;
		}
		else
		{
			...
			m_conf.ps.rov_depth = depth_write ?
				...
				GSHWDrawConfig::PS_ROV_DEPTH::READ_ONLY;
			m_conf.rov_depth = GSHWDrawConfig::CONTINUOUS_ROV;
		}
	...
	if (rov_color)
	{
		...
		if (oneshot)
		{
			m_conf.ps.rov_color = GSHWDrawConfig::PS_ROV_COLOR::ONESHOT_ENABLED;
			m_conf.rov_color = GSHWDrawConfig::ONESHOT_ROV;
		}
		else
		{
			m_conf.ps.rov_color = GSHWDrawConfig::PS_ROV_COLOR::ENABLED;
			m_conf.rov_color = GSHWDrawConfig::CONTINUOUS_ROV;
		}
	}
...
void GSRendererHW::ConvertTextureTypeROV(GSTextureCache::Target* rt, GSTextureCache::Target* ds)
{
	...
	if (ds)
	{
		if (m_conf.HasContinuousDepthROV() && !ds->m_texture->IsShaderWrite())
		{
			GL_PUSH("HW: Convert DepthStencil -> DepthColor for ROV.");
			ConvertTextureTypeROVSingle(ds, true);
		}
		else if (!m_conf.HasContinuousDepthROV() && !ds->m_texture->IsDepthStencil())
		{
	...
	if (rt && m_conf.HasContinuousColorROV() && !rt->m_texture->IsShaderWrite())
	{
