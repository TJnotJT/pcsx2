...
GSRendererHW::TextureShuffleResult GSRendererHW::DetectTextureShuffleImpl()
{
	...
	if (quad_mixes_16pixel_groups)
	{
		...
		{
			b_clobber = !!(shuffle_channels & TextureShuffleChannels_WriteBlue);
			a_clobber = !!(shuffle_channels & TextureShuffleChannels_WriteAlpha);
		}
		else if ((x_page_offset == 8 && u_page_offset == 0) ||
			(x_page_offset == 0 && u_page_offset == -8))
		{
			r_clobber = !!(shuffle_channels & TextureShuffleChannels_WriteRed);
...
GSRendererHW::TextureShuffleResult GSRendererHW::DetectTextureShuffle()
{
...
void GSRendererHW::ConvertSpriteTextureShuffleImpl(GSTextureCache::Target* rt, GSTextureCache::Source* tex)
{
	...
	if (m_split_texture_shuffle_pages > 0 ||
		m_texture_shuffle_result.type == TextureShuffleType::TwoPixel)
	{
		half_x = false;
		half_y = false;
		half_u = false;
	...
	else if (m_texture_shuffle_result.type == TextureShuffleType::GappedSwizzle)
	{
...
void GSRendererHW::Draw()
{
	...
	m_texture_shuffle_result = DetectTextureShuffle();
	...
	// Add a function to end texture shuffle.
	...
	if (m_process_texture)
	{
		...
		if (rt)
		{
			if (m_texture_shuffle_result && DetectTextureShuffleSecondPass(rt, src))
			{
				...
				m_same_group_texture_shuffle = (m_texture_shuffle_result.channels & TextureShuffleChannels_BlueToAlpha);
			...
			{
				m_texture_shuffle_result.type = TextureShuffleType::None;
			}
		}
		...
		const bool is_shuffle = m_channel_shuffle || m_texture_shuffle_result;
	...
	if (rt)
	{
		...
		rt->m_32_bits_fmt = m_texture_shuffle_result || (GSLocalMemory::m_psm[m_cached_ctx.FRAME.PSM].bpp != 16);
	}
	...
	const bool can_update_size = !is_possible_mem_clear && !m_texture_shuffle_result && !m_channel_shuffle;

	if (!m_texture_shuffle_result && !m_channel_shuffle)
	{
	...
	else if (!m_texture_shuffle_result)
	{
	...
	if (rt)
		rt->UpdateValidChannels(rt->m_TEX0.PSM, m_texture_shuffle_result ? GetEffectiveTextureShuffleFbmsk() : fm);
	if (ds)
	...
	if (!(m_cached_ctx.TEST.DATE && m_cached_ctx.TEST.DATM))
	{
		...
		if (src && m_texture_shuffle_result && !m_copy_16bit_to_target_shuffle)
		{
		...
		else if (m_texture_shuffle_result && buffer_width > 64 && update_rect.z > buffer_width)
		{
		...
		if (rt)
		{
			...
			const bool rt_update = can_update_size || (is_possible_mem_clear && m_vt.m_min.c.a > 0) || (m_texture_shuffle_result && (src && src->m_from_target != rt));
			...
			rt->UpdateValidity(update_rect, !frame_masked && (rt_update || (m_r.w <= (resolution.y * 2) && !m_texture_shuffle_result)));
			rt->UpdateDrawn(update_rect, !frame_masked && (rt_update || (m_r.w <= (resolution.y * 2) && !m_texture_shuffle_result)));
		...
		if (ds)
		{
			...
			if (rt && m_using_temp_z)
			{
				...
				ds->UpdateValidity(ds_rect, z_update && (can_update_size || (ds_rect.w <= (resolution.y * 2) && !m_texture_shuffle_result)));
				ds->UpdateDrawn(ds_rect, z_update && (can_update_size || (ds_rect.w <= (resolution.y * 2) && !m_texture_shuffle_result)));
			}
			else
			{
	...
	if (!m_texture_shuffle_result && !m_channel_shuffle && rt && src && src->m_from_target == rt && src->m_target_direct && rt->m_texture == src->m_texture)
	{
	...
	if (rt)
	{
		...
		if (rt->m_last_draw >= s_n || m_texture_shuffle_result || m_channel_shuffle || (!rt->m_dirty.empty() && !rt->m_dirty.GetTotalRect(rt->m_TEX0, rt->m_unscaled_size).rintersect(m_r).rempty()))
		{
			const u32 alpha = m_cached_ctx.FRAME.FBMSK >> 24;
			const u32 alpha_mask = GSLocalMemory::m_psm[m_cached_ctx.FRAME.PSM].fmsk >> 24;
			rt->Update(m_texture_shuffle_result || (alpha != 0 && (alpha & alpha_mask) != alpha_mask) || (!alpha && (GetAlphaMinMax().max | (m_context->FBA.FBA << 7)) > 128));
		}
		else
			rt->m_age = 0;
	}
	if (ds)
	{
		if (ds->m_last_draw >= s_n || m_texture_shuffle_result || m_channel_shuffle || (!ds->m_dirty.empty() && !ds->m_dirty.GetTotalRect(ds->m_TEX0, ds->m_unscaled_size).rintersect(m_r).rempty()))
			ds->Update();
		else
			ds->m_age = 0;
	}
	...
	if ((fm & fm_mask) != fm_mask && rt)
	{
		...
		// Limit to 2x the vertical height of the resolution (for double buffering)
		rt->UpdateValidity(real_rect, !frame_masked && (can_update_size || (real_rect.w <= (resolution.y * 2) && !m_texture_shuffle_result)));
	...
	if (ds)
	{
		...
		if (m_using_temp_z)
		{
			...
			if (was_written)
			{
				...
				ds->UpdateValidity(ds_real_rect, !z_masked && (can_update_size || (ds_real_rect.w <= (resolution.y * 2) && !m_texture_shuffle_result)));
			}
		...
		{
			ds->UpdateValidity(real_rect, !z_masked && (can_update_size || (real_rect.w <= (resolution.y * 2) && !m_texture_shuffle_result)));
			ds->UpdateDrawn(real_rect, !z_masked && (can_update_size || (real_rect.w <= (resolution.y * 2) && !m_texture_shuffle_result)));
	...
	if ((fm & fm_mask) != fm_mask && !no_rt)
	{
		...
		g_texture_cache->InvalidateVideoMemType(GSTextureCache::DepthStencil, m_cached_ctx.FRAME.Block(),
			m_cached_ctx.FRAME.PSM, m_texture_shuffle_result ? GetEffectiveTextureShuffleFbmsk() : fm);
...
void GSRendererHW::EmulateDATESelectMethod(DATEOptions& date_options, GSTextureCache::Target* rt, int& blend_alpha_min, int& blend_alpha_max)
{
	...
	else if ((features.texture_barrier && m_prim_overlap == PRIM_OVERLAP_NO) || m_texture_shuffle_result)
	{
...
void GSRendererHW::EmulateTextureShuffleAndFbmask(GSTextureCache::Target* rt, GSTextureCache::Source* tex)
{
	...
	if (m_texture_shuffle_result)
	{
		...
		if (m_texture_shuffle_result.channels & TextureShuffleChannels_ReadRedGreen)
		{
			process_rg |= SHUFFLE_READ;
		}

		if (m_texture_shuffle_result.channels & TextureShuffleChannels_ReadBlueAlpha)
		{
			process_ba |= SHUFFLE_READ;
		}

		if (m_texture_shuffle_result.channels & TextureShuffleChannels_WriteRedGreen)
		{
			process_rg |= SHUFFLE_WRITE;
		}

		if (m_texture_shuffle_result.channels & TextureShuffleChannels_WriteBlueAlpha)
		{
			process_ba |= SHUFFLE_WRITE;
		}

		shuffle_across = !!(m_texture_shuffle_result.channels & TextureShuffleChannels_ShuffleAcross);
		...
		m_conf.ps.shuffle_same = !!(m_texture_shuffle_result.channels & TextureShuffleChannels_BlueToAlpha);
		...
		if (!(m_texture_shuffle_result.channels & TextureShuffleChannels_WriteRed))
			fbmask_r = 0xFF;
		if (!(m_texture_shuffle_result.channels & TextureShuffleChannels_WriteGreen))
			fbmask_g = 0xFF;
		if (!(m_texture_shuffle_result.channels & TextureShuffleChannels_WriteBlue))
			fbmask_b = 0xFF;
		if (!(m_texture_shuffle_result.channels & TextureShuffleChannels_WriteAlpha))
			fbmask_a = 0xFF;
		...
		const auto AllOnesOrZeros = [](int x, int mask) {
			return (x & mask) == 0 || (x & mask) == mask;
		};
...
__ri void GSRendererHW::DrawPrims(GSTextureCache::Target* rt, GSTextureCache::Target* ds, GSTextureCache::Source* tex, const TextureMinMaxResult& tmm)
{
	...
	if (rt)
	{
		...
		if (GSUtil::GetChannelMask(m_cached_ctx.FRAME.PSM) & 0x8 && !m_texture_shuffle_result)
		{
		...
		else if ((m_texture_shuffle_result && m_conf.colormask.wa))
		{
	...
	if (rt)
	{
		...
		const bool rta_decorrection = m_channel_shuffle || m_texture_shuffle_result || (m_conf.colormask.wa && (rt_new_alpha_max > 128 || partial_fbmask));
	...
	if ((!IsOpaque() || m_context->ALPHA.IsBlack()) && rt && ((m_conf.colormask.wrgba & 0x7) || (m_texture_shuffle_result && !m_copy_16bit_to_target_shuffle && !m_same_group_texture_shuffle)))
	{
...
{
	// No shuffles.
	if (m_channel_shuffle || m_texture_shuffle_result)
