#include "GS/Renderers/HW/GSTextureCacheHybrid.h"
...
GSTextureCacheHybrid::GSTextureCacheHybrid()
{
}

GSTextureCacheHybrid::GSCachedTexture::~GSCachedTexture()
{
	if (m_texture)
	{
...
GSTextureCacheHybrid::GSCachedTexture::GSCachedTexture(const GSMemoryRect& rect, const GSMemoryCLUT& clut)
	: m_texture{}
	, m_clut{}
	, m_mem_rect(rect)
	, m_mem_clut(clut)
...
void GSTextureCacheHybrid::GSCachedTexture::WritebackIfDirty()
{
	if (IsDirty())
	{
...
bool GSTextureCacheHybrid::GSCachedTexture::Matches(const GSMemoryRect& rect, const GSMemoryCLUT& clut) const
{
	return (m_mem_rect == rect) && (!HasCLUT() || clut == m_mem_clut);
}

GSTextureCacheHybrid::GSCachedTexture* GSTextureCacheHybrid::LookupTexture(const GSMemoryRect& rect, const GSMemoryCLUT& clut)
{
...
void GSTextureCacheHybrid::WritebackAllTextures()
{
