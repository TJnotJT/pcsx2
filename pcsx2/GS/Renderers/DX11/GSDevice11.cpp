...
void GSDevice11::RenderHW(GSHWDrawConfig& config)
{
	...
	GSTexture* draw_rt = config.HasContinuousColorROV() ? nullptr : config.rt;
	GSTexture* draw_ds_as_rt = m_ds_as_rt;
	GSTexture* draw_ds = config.HasContinuousDepthROV() ? nullptr : config.ds;
	GSTexture* draw_rt_rov = config.HasContinuousColorROV() ? config.rt : nullptr;
	GSTexture* draw_ds_rov = config.HasContinuousDepthROV() ? config.ds : nullptr;
