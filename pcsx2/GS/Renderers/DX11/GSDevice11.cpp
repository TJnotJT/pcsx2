	...
	const GSVector4i* scissor, ID3D11DepthStencilView* read_only_dsv)
{
	...
	ID3D11RenderTargetView* dsrtv = nullptr;
	...
	if (ds_as_rt)
	{
		...
		dsrtv = *static_cast<GSTexture11*>(ds_as_rt);
	}
	if (ds)
	{
		CommitClear(ds);
	...
	const bool changed = (m_state.rtv != rtv || m_state.dsrtv != dsrtv || m_state.dsv != dsv ||
		m_state.rt_uav != rt_uav || m_state.ds_uav != ds_uav);
	...
	if (m_state.dsrtv != dsrtv)
	{
		if (m_state.dsrtv)
			m_state.dsrtv->Release();
		if (dsrtv)
			dsrtv->AddRef();
		m_state.dsrtv = dsrtv;
	...
	PSUnbindConflictingSRVs(m_state.current_rt, m_state.current_ds_as_rt, read_only_dsv ? nullptr : m_state.current_ds);
...
void GSDevice11::RenderHW(GSHWDrawConfig& config)
{
	...
	if (!(draw_rt || draw_rt_rov) && draw_ds && m_state.rtv && m_state.current_rt && m_state.rtv == *static_cast<GSTexture11*>(m_state.current_rt) &&
		m_state.current_ds == draw_ds && config.tex != m_state.current_rt && m_state.current_rt->GetSize() == draw_ds->GetSize() && !draw_ds_as_rt)
	{
		...
		m_state.current_rt == draw_rt && config.tex != m_state.current_ds && m_state.current_ds->GetSize() == draw_rt->GetSize() && !draw_ds_as_rt)
	{
	...
	const bool ds_feedbackloop_pass1 = draw_ds && config.IsFeedbackLoopDepth(config.ps);
	const bool ds_feedbackloop_pass2 = draw_ds && config.IsFeedbackLoopDepth(config.alpha_second_pass.ps);
	...
	const bool ds_as_rt_feedbackloop_pass1 = draw_ds_as_rt && config.IsFeedbackLoopDepth(config.ps);
	const bool ds_as_rt_feedbackloop_pass2 = draw_ds_as_rt && config.IsFeedbackLoopDepth(config.alpha_second_pass.ps);
	if (draw_ds_as_rt && !draw_ds_rov && (config.require_one_barrier || (config.require_full_barrier && m_features.multidraw_fb_copy)) &&
		(ds_as_rt_feedbackloop_pass1 || ds_as_rt_feedbackloop_pass2))
	{
		...
		if (!draw_ds_as_rt_clone)
			Console.Warning("D3D11: Failed to allocate temp texture for DS as RT copy.");
	}
	...
	SendHWDraw(config,
		rt_feedbackloop_pass1 ? draw_rt_clone : nullptr, draw_rt,
...
{
	const GSVector4i rtsize = (rt ? rt : (ds_as_rt ? ds_as_rt : ds))->GetRect();
