...
u32 D3D12CompilerAsync::GetCompileResultsAsync(CompileJob* out, u32 max_count)
{
	...
	while (m_done_jobs > 0 && i < max_count)
	{
		...
		pxAssert(m_queued_jobs > 0);
		pxAssert(m_done_jobs > 0);
		pxAssert(m_done_jobs <= m_queued_jobs);
...
void D3D12CompilerAsync::StartCompileJobAsync(CompileJob job)
{
	...
	if (m_queued_jobs < MAX_JOBS)
	{
		...
		pxAssert(m_queued_jobs <= MAX_JOBS);
		pxAssert(m_done_jobs <= m_queued_jobs);
	...
	while (true)
	{
		u32 done = 0;
		{
			...
			m_worker_cv.wait(lock, [&]() {
				return m_stop || m_done_jobs < m_queued_jobs;
			});
...

		{
			...
			m_job_done = (m_job_done + 1) % MAX_JOBS;
			m_done_jobs++;

			pxAssert(m_done_jobs <= MAX_JOBS);
			pxAssert(m_done_jobs <= m_queued_jobs);
		}
	}
}
