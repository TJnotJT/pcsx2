...
bool GSDevice12::CheckFeatures(const u32& vendor_id)
{
	...
	m_features.rov_depth = options.ROVsSupported;
	...
	const GSVector2i& viewport_size)
{
	...
	if (!InRenderPass())
	{
		...
		if (d12Ds)
			d12Ds->TransitionToState(D3D12_RESOURCE_STATE_DEPTH_WRITE);
	}
...
void GSDevice12::InvalidateCachedState()
{
	...
	m_tfx_uav_textures_handle_gpu.Clear();
}
...
void GSDevice12::PSSetUAV(int i, GSTexture* uav, bool check_state)
{
	...
	if (uav)
	{
		...
		if (check_state)
		{
			if (dtex->GetTargetMode() != GSTexture12::TargetMode::UAV)
			{
				if (InRenderPass())
				{
					GL_INS("Ending render pass due to UAV transition");
					EndRenderPass();
				}
				dtex->CommitClear();
				dtex->SetTargetMode(GSTexture12::TargetMode::UAV);
			}
		}
		dtex->SetUseFenceCounter(GetCurrentFenceValue());
	...
	D3D12_RENDER_PASS_ENDING_ACCESS_TYPE stencil_end, GSVector4 clear_color, float clear_depth, u8 clear_stencil)
{
	...
	GetCommandList()->BeginRenderPass(num_rts, rt.data(), m_current_depth_target ? &ds : nullptr, D3D12_RENDER_PASS_FLAG_NONE);
}
...
bool GSDevice12::ApplyTFXState(bool already_execed)
{
	...
	if (flags & DIRTY_FLAG_TFX_UAV_TEXTURES)
	{
		if (!GetTextureGroupDescriptors(&m_tfx_uav_textures_handle_gpu, m_tfx_uavs.data(), 2))
		{
			ExecuteCommandListAndRestartRenderPass(false, "Ran out of TFX UAV descriptor descriptor groups");
			return ApplyTFXState(true);
		}
		flags |= DIRTY_FLAG_TEXTURES_DESCRIPTOR_TABLE_4;
	}
	...
	if (m_current_root_signature != RootSignature::TFX)
	{
		...
		flags |= DIRTY_FLAG_VS_CONSTANT_BUFFER_BINDING | DIRTY_FLAG_PS_CONSTANT_BUFFER_BINDING |
		         ...
		         DIRTY_FLAG_TEXTURES_DESCRIPTOR_TABLE_4 | DIRTY_FLAG_PIPELINE;
	...
	if (flags & DIRTY_FLAG_TEXTURES_DESCRIPTOR_TABLE_4)
		cmdlist->SetGraphicsRootDescriptorTable(TFX_ROOT_SIGNATURE_PARAM_PS_UAV_TEXTURES, m_tfx_uav_textures_handle_gpu);
...
void GSDevice12::RenderHW(GSHWDrawConfig& config)
{
	...
	const bool feedback_rt = draw_rt &&
		(config.require_one_barrier || (config.require_full_barrier && m_features.texture_barrier) ||
