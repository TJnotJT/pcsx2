...
void GSDevice12::ExecuteAndRestartRenderPass(std::function<void(void)> exec)
{
...
void GSDevice12::ExecuteCommandListAndRestartRenderPass(bool wait_for_completion, const char* reason)
{
	...
	ExecuteAndRestartRenderPass(exec);
}
	...
	if (draw_rt_clone || draw_ds_clone)
	{
		...
		if (m_features.multidraw_fb_copy && full_barrier)
		{
			...
			for (u32 n = 0, p = 0; n < draw_list_size; n++)
			{
				...
				ExecuteAndRestartRenderPass([&]() { CopyAndBind(bbox); });
				if (BindDrawPipeline(pipe))
					Draw(p, count);
				p += count;
			}
		...
		if (!skip_first_barrier)
			ExecuteAndRestartRenderPass([&]() { CopyAndBind(config.drawarea); });
	}
