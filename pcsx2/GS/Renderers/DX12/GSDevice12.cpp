...
void GSDevice12::RenderHW(GSHWDrawConfig& config)
{
	...
	GSTexture12* draw_rt = config.HasContinuousColorROV() ? nullptr : static_cast<GSTexture12*>(config.rt);
	GSTexture12* draw_ds = config.HasContinuousDepthROV() ? nullptr : static_cast<GSTexture12*>(config.ds);
	GSTexture12* draw_rt_rov = config.HasContinuousColorROV() ? static_cast<GSTexture12*>(config.rt) : nullptr;
	GSTexture12* draw_ds_rov = config.HasContinuousDepthROV() ? static_cast<GSTexture12*>(config.ds) : nullptr;
	...
	if (colclip_rt)
	{
		if (config.colclip_mode == GSHWDrawConfig::ColClipMode::EarlyResolve)
		{
			...
			colclip_rt = nullptr;
			draw_rt = config.HasContinuousColorROV() ? nullptr : static_cast<GSTexture12*>(config.rt);
		}
		else
		{
			draw_rt = colclip_rt;
	...
	OMSetRenderTargets(draw_rt, draw_ds_as_rt, draw_ds, config.scissor,
		config.tex && (config.tex == draw_ds || config.ps.IsFeedbackLoopDepth()) && !config.depth.zwe && !config.HasContinuousDepthROV(),
...
void GSDevice12::UpdateHWPipelineSelector(GSHWDrawConfig& config)
{
	...
	m_pipeline_selector.dss.key = config.HasContinuousDepthROV() ? GSHWDrawConfig::DepthStencilSelector::NoDepth().key : config.depth.key;
	m_pipeline_selector.bs.key = config.HasContinuousColorROV() ? GSHWDrawConfig::BlendState().key : config.blend.key;
	m_pipeline_selector.bs.constant = 0; // don't dupe states with different alpha values
	m_pipeline_selector.cms.key = config.HasContinuousColorROV() ? GSHWDrawConfig::ColorMaskSelector().key : config.colormask.key;
	m_pipeline_selector.topology = static_cast<u32>(config.topology);
	m_pipeline_selector.rt = config.rt != nullptr && !config.HasContinuousColorROV();
	m_pipeline_selector.ds = config.ds != nullptr && !config.HasContinuousDepthROV();
	m_pipeline_selector.ds_as_rt = m_ds_as_rt != nullptr && !config.HasContinuousDepthROV();
}
