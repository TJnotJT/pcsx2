...
GSTexture12* GSDevice12::SetupPrimitiveTrackingDATE(GSHWDrawConfig& config, PipelineSelector& pipe)
{
	...
	if ((init_pipe.vs.expand == GSHWDrawConfig::VSExpand::LineZInt) ||
		(init_pipe.vs.expand == GSHWDrawConfig::VSExpand::TriangleZInt) ||
		(init_pipe.vs.expand == GSHWDrawConfig::VSExpand::PointZInt))
	{
		init_pipe.vs.expand = GSHWDrawConfig::VSExpand::None;
	}
...
void GSDevice12::RenderHW(GSHWDrawConfig& config)
{
	...
	const bool feedback_depth = config.ps.depth_feedback;
...
void GSDevice12::UpdateHWPipelineSelector(GSHWDrawConfig& config)
{
	...
	if (config.ds_as_rt)
	{
		switch (config.ds_as_rt->GetFormat())
		{
