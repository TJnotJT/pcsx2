...
GSTexture12::GSTexture12(Type type, Format format, ShaderAccess access, int width, int height, int levels, DXGI_FORMAT dxgi_format,
...
std::unique_ptr<GSTexture12> GSTexture12::Create(Type type, Format format, ShaderAccess access, int width, int height, int levels,
	...
	DXGI_FORMAT uav_format)
{
	...
	switch (type)
	{
		...
		case Type::DepthStencil:
		{
			...
			pxAssert(IsShaderReadOnly(access) && uav_format == DXGI_FORMAT_UNKNOWN);
		}
		break;
	...
	pxAssert(IsShaderReadWrite(access) == (uav_format != DXGI_FORMAT_UNKNOWN));
	if (IsShaderReadWrite(access))
	{
		pxAssert(!IsDepthStencil(type));
	...
	return std::unique_ptr<GSTexture12>(
		new GSTexture12(type, format, access, width, height, levels, dxgi_format, std::move(resource), std::move(resource_fbl), std::move(allocation),
...
std::unique_ptr<GSTexture12> GSTexture12::Adopt(wil::com_ptr_nothrow<ID3D12Resource> resource, Type type, Format format,
	ShaderAccess access, int width, int height, int levels, DXGI_FORMAT dxgi_format, DXGI_FORMAT srv_format, DXGI_FORMAT rtv_format,
	DXGI_FORMAT dsv_format, DXGI_FORMAT uav_format, ResourceState resource_state)
{
	...
	return std::unique_ptr<GSTexture12>(new GSTexture12(type, format, access, static_cast<u32>(desc.Width), desc.Height,
