...
public:
	...
	static std::unique_ptr<GSTextureVK> Create(Type type, Format format, ShaderAccess access, int width, int height, int levels);
	...
	bool IsShaderReadWriteMode() const override { return GetLayout() == Layout::ReadWriteImage; }
...
private:
	GSTextureVK(Type type, Format format, ShaderAccess access, int width, int height, int levels, VkImage image, VmaAllocation allocation,
