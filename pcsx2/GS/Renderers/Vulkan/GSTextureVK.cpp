...
void GSTextureVK::UpdateDepthColor(bool color_to_ds)
{
	...
	if (color_to_ds)
	{
		...
		if (GetState() != State::Cleared)
		{
			static_cast<GSTextureVK*>(m_depth_color.get())->TransitionToLayout(Layout::ShaderReadOnly);
	...
	else
	{
		...
		if (GetState() != State::Cleared)
		{
			static_cast<GSTextureVK*>(m_depth_color.get())->TransitionToLayout(Layout::ColorAttachment);
