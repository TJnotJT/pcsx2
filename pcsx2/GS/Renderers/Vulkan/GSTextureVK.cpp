...
std::unique_ptr<GSTextureVK> GSTextureVK::Create(Usage usage, Format format, int width, int height, int levels)
{
	...
	ici.extent = {.width = static_cast<u32>(width), .height = static_cast<u32>(height), .depth = 1};
	...
	u32 buffer_height, u32 row_length, VkBuffer buffer, u32 buffer_offset)
{
	...
	bic.imageOffset = {.x = static_cast<s32>(x), .y = static_cast<s32>(y), .z = 0};
	bic.imageExtent = {.width = width, .height = height, .depth = 1u};
...
void GSTextureVK::GetLinkedFramebuffer(FramebufferInfo& info)
{
	...
	const VkRenderPass rp = GSDeviceVK::GetInstance()->GetRenderPass(
		...
		info.ds_as_rt ? info.ds_as_rt->GetVkFormat() : VK_FORMAT_UNDEFINED,
