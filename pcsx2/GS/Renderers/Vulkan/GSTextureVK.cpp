...
GSTextureVK::GSTextureVK(Type type, Format format, ShaderAccess access, int width, int height, int levels, VkImage image,
...
std::unique_ptr<GSTextureVK> GSTextureVK::Create(Type type, Format format, ShaderAccess access, int width, int height, int levels)
{
	...
	if (IsRenderTargetOrDepthStencil(type) && width >= 512 && height >= 448)
	...
	return std::unique_ptr<GSTextureVK>(
		new GSTextureVK(type, format, access, width, height, levels, image, allocation, view, vk_format));
}

std::unique_ptr<GSTextureVK> GSTextureVK::Adopt(
	VkImage image, Type type, Format format, ShaderAccess access, int width, int height, int levels, VkFormat vk_format)
{
	...
	return std::unique_ptr<GSTextureVK>(
		new GSTextureVK(type, format, access, width, height, levels, image, VK_NULL_HANDLE, view, vk_format));
}
