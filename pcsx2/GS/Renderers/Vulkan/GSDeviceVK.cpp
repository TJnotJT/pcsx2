...
void GSDeviceVK::SetupAccurateLines(GSHWDrawConfig& config)
{
	...
	if (config.accurate_lines_data)
	{
		...
		{
			ExecuteCommandBufferAndRestartRenderPass(false, "Uploading bytes to accurate lines buffer");
			if (!m_accurate_lines_stream_buffer.ReserveMemory(size, sizeof(AccurateLinesData)))
...
bool GSDeviceVK::CreateBuffers()
{
	...
	{
		if (!m_accurate_lines_stream_buffer.Create(VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, ACCURATE_LINES_BUFFER_SIZE))
		{
...
bool GSDeviceVK::CreatePersistentDescriptorSets()
{
	...
	if (m_features.accurate_lines)
	{
		dsub.AddBufferDescriptorWrite(m_tfx_ubo_descriptor_set, 3, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
			m_accurate_lines_stream_buffer.GetBuffer(), 0, ACCURATE_LINES_BUFFER_SIZE);
	}
	dsub.Update(dev);
