	...
	bool depth_sampling)
{
	return RenderPass(color_format, depth_format, color_load_op,
		...
		stencil_load_op, stencil_store_op, color_feedback_loop, depth_sampling);
}
...
VkRenderPass GSDeviceVK::GetRenderPassVK(const RenderPass& rp)
{
	...
	rpb.SetUseFeedbackLoopLayout(UseFeedbackLoopLayout());
	rpb.SetUseFrameBufferFetch(m_optional_extensions.vk_ext_rasterization_order_attachment_access);

	for (u32 i = 0; i < MAX_COLOR_ATTACHMENTS; i++)
	{
		if (rp.GetColorFormat(i) != VK_FORMAT_UNDEFINED)
		{
			rpb.AddColorAttachment(rp.GetColorFormat(i), rp.GetColorLoadOp(i), rp.GetColorStoreOp(i), rp.GetColorFeedbackLoop());
		}
	}

	if (rp.GetDepthFormat() != VK_FORMAT_UNDEFINED)
	{
		rpb.AddDepthStencilAttachment(rp.GetDepthFormat(), rp.GetDepthLoadOp(), rp.GetDepthStoreOp(),
			rp.GetStencilLoadOp(), rp.GetStencilStoreOp(), rp.GetDepthSampling());
	}
...
#define GET(dest, rt, depth, fbl, dsp, opa, opb, opc) \
	...
	if (m_date_setup_render_pass.IsNull())
		return false;
...
VkRenderingAttachmentInfo GSDeviceVK::GetRenderingAttachmentInfo(VkImageView view, VkImageLayout layout,
...
void GSDeviceVK::BeginDynamicRenderPass(const RenderPass& rp, const GSVector4i& rect, const VkClearValue* cv, u32 cv_count)
{
	...
	{
		color[i] = GetRenderingAttachmentInfo(
			m_current_render_target->GetView(), m_current_render_target->GetVkLayout(),
	...
	{
		depth = GetRenderingAttachmentInfo(
			m_current_depth_target->GetView(), m_current_depth_target->GetVkLayout(),
			rp.GetDepthLoadOp(), rp.GetDepthStoreOp(), (cv && i < cv_count) ? cv[i] : VkClearValue{}); 
		depth_ptr = &depth;

		if (m_features.stencil_buffer)
		{
			stencil = GetRenderingAttachmentInfo(
				m_current_depth_target->GetView(), m_current_depth_target->GetVkLayout(),
				rp.GetStencilLoadOp(), rp.GetStencilStoreOp(), (cv && i < cv_count) ? cv[i] : VkClearValue{});
			stencil_ptr = &stencil;
		}
...
bool GSDeviceVK::BeginPresentRenderPass(const RenderPass& rp, const GSVector4i& rect, GSTextureVK* swap_chain)
{
	...
	if (UseDynamicRendering())
	{
		VkRenderingAttachmentInfo color = GetRenderingAttachmentInfo(
			swap_chain->GetView(), swap_chain->GetVkLayout(), 
	...
	else
	{
		...
		const VkRenderPassBeginInfo begin_info = { VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO, nullptr,
			GetRenderPassVK(m_current_render_pass),
			fb, {{rect.x, rect.y}, {static_cast<u32>(rect.width()), static_cast<u32>(rect.height())}},
...
VkImageMemoryBarrier GSDeviceVK::GetColorFeedbackBarrier(GSTextureVK* rt) const
{
	const VkImageLayout layout = Vulkan::RenderPassBuilder::GetColorLayout(true, UseFeedbackLoopLayout());
	return VkImageMemoryBarrier{
		.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER,
		.pNext = nullptr,
		.srcAccessMask = Vulkan::RenderPassBuilder::GetColorFeedbackLoopSrcAccessMask(),
		.dstAccessMask = Vulkan::RenderPassBuilder::GetColorFeedbackLoopDstAccessMask(UseFeedbackLoopLayout()),
		.oldLayout = layout,
		.newLayout = layout,
...
VkImageMemoryBarrier GSDeviceVK::GetDepthFeedbackBarrier(GSTextureVK* ds) const
{
	const VkImageLayout layout = Vulkan::RenderPassBuilder::GetDepthLayout(true, UseFeedbackLoopLayout());
	return VkImageMemoryBarrier{
		.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER,
		.pNext = nullptr,
		.srcAccessMask = Vulkan::RenderPassBuilder::GetDepthFeedbackLoopSrcAccessMask(),
		.dstAccessMask = Vulkan::RenderPassBuilder::GetDepthFeedbackLoopDstAccessMask(UseFeedbackLoopLayout()),
		.oldLayout = layout,
		.newLayout = layout,
		.srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED,
		.dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED,
...
{
	return Vulkan::RenderPassBuilder::GetFeedbackLoopDependencyFlags(UseFeedbackLoopLayout());
}
...
void GSDeviceVK::FeedbackBarrier(GSTextureVK* rt, GSTextureVK* ds)
{
	...
	if (rt)
	{
		VkImageMemoryBarrier barrier = GetColorFeedbackBarrier(rt);
		vkCmdPipelineBarrier(GetCurrentCommandBuffer(),
			Vulkan::RenderPassBuilder::GetColorFeedbackLoopSrcStageMask(),
			Vulkan::RenderPassBuilder::GetColorFeedbackLoopDstStageMask(),
			barrier_flags, 0, nullptr, 0, nullptr, 1, &barrier);
	}

	if (ds)
	{
		VkImageMemoryBarrier barrier = GetDepthFeedbackBarrier(ds);
		vkCmdPipelineBarrier(GetCurrentCommandBuffer(),
			Vulkan::RenderPassBuilder::GetDepthFeedbackLoopSrcStageMask(),
			Vulkan::RenderPassBuilder::GetDepthFeedbackLoopDstStageMask(),
			barrier_flags, 0, nullptr, 0, nullptr, 1, &barrier);
	}
}
