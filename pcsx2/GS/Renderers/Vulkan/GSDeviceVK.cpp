...
bool GSDeviceVK::CheckFeatures()
{
	...
	for (u32 fmt = static_cast<u32>(GSTexture::Format::Color); fmt < static_cast<u32>(GSTexture::Format::PrimID); fmt++)
	{
		...
		if (GSTexture::IsFormatUsedForRW(static_cast<GSTexture::Format>(fmt)))
...
{
	const GSTexture::Type null_type = m_features.rov ? GSTexture::Type::RWRenderTarget : GSTexture::Type::RenderTarget;
	m_null_texture = GSTextureVK::Create(null_type, GSTexture::Format::Color, 1, 1, 1);
	if (!m_null_texture)
		return false;
...
void GSDeviceVK::PSSetShaderResourceRW(GSTexture* rt, GSTexture* ds, bool write_rt, bool write_ds)
{
	GSTextureVK* vkRt = static_cast<GSTextureVK*>(rt);
	GSTextureVK* vkDs = static_cast<GSTextureVK*>(ds);
...
void GSDeviceVK::RenderHW(GSHWDrawConfig& config)
{
	...
	PSSetShaderResourceRW(draw_rt_rov, draw_ds_rov, config.ps.HasColorOutput(), config.ps.HasDepthROVWrite());
