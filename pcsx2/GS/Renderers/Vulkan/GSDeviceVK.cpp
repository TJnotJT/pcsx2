...
bool GSDeviceVK::SelectDeviceFeatures()
{
	...
	VkPhysicalDeviceFeatures available_features = available_features2.features;
...
bool GSDeviceVK::CreateDevice(VkSurfaceKHR surface, bool enable_validation_layer)
{
	...
	std::vector<VkQueueFamilyProperties2> queue_family_properties(queue_family_count);
	std::fill(queue_family_properties.begin(), queue_family_properties.end(),
	...
	{
		VkBool32 graphics_supported = queue_family_properties[i].queueFamilyProperties.queueFlags & VK_QUEUE_GRAPHICS_BIT;
	...
	for (uint32_t i = 0; i < queue_family_count; i++)
	{
		...
		if (!(queue_family_properties[i].queueFamilyProperties.queueFlags & VK_QUEUE_COMPUTE_BIT))
			continue; // We need compute
		if (queue_family_properties[i].queueFamilyProperties.timestampValidBits == 0)
		...
		if (queue_is_used && queue_family_properties[i].queueFamilyProperties.queueCount > 1)
			spin_queue_index = 1;
