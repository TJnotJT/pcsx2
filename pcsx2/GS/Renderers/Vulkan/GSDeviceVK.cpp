...
void GSDeviceVK::RenderHW(GSHWDrawConfig& config)
{
	...
	GSTextureVK* draw_rt = config.HasContinuousColorROV() ? nullptr : static_cast<GSTextureVK*>(config.rt);
	GSTextureVK* draw_ds = config.HasContinuousDepthROV() ? nullptr : static_cast<GSTextureVK*>(config.ds);
	GSTextureVK* draw_rt_rov = config.HasContinuousColorROV() ? static_cast<GSTextureVK*>(config.rt) : nullptr;
	GSTextureVK* draw_ds_rov = config.HasContinuousDepthROV() ? static_cast<GSTextureVK*>(config.ds) : nullptr;
	...
	if (colclip_rt)
	{
		if (config.colclip_mode == GSHWDrawConfig::ColClipMode::EarlyResolve)
		{
			...
			colclip_rt = nullptr;
			draw_rt = config.HasContinuousColorROV() ? nullptr : static_cast<GSTextureVK*>(config.rt);
		}
		else
		{
...
void GSDeviceVK::UpdateHWPipelineSelector(GSHWDrawConfig& config, PipelineSelector& pipe)
{
	...
	pipe.dss.key = config.HasContinuousDepthROV() ? GSHWDrawConfig::DepthStencilSelector::NoDepth().key : config.depth.key;
	pipe.bs.key = config.HasContinuousColorROV() ? GSHWDrawConfig::BlendState().key : config.blend.key;
	pipe.bs.constant = 0; // don't dupe states with different alpha values
	pipe.cms.key = config.HasContinuousColorROV() ? GSHWDrawConfig::ColorMaskSelector().key : config.colormask.key;
	pipe.topology = static_cast<u32>(config.topology);
	pipe.rt = config.rt != nullptr && !config.HasContinuousColorROV();
	pipe.ds = config.ds != nullptr && !config.HasContinuousDepthROV();
	...
	bool one_barrier, bool full_barrier)
{
	...
	if (config.HasContinuousROV() || config.HasOneshotROV())
		g_perfmon.Put(GSPerfMon::DrawCallsROV, 1);
}
