	...
	GSTexture* rt, GSTexture* ds, const GSVector4i& scissor, FeedbackLoopFlag feedback_loop)
{
		...
		if (vkRt)
		{
			m_current_framebuffer =
				vkRt->GetLinkedFramebuffer(vkDs,
					...
					(feedback_loop & FeedbackLoopFlag_ReadAndWriteDS) != 0);
		}
		else
		{
			pxAssert(!(feedback_loop & FeedbackLoopFlag_ReadAndWriteRT) &&
					 !(feedback_loop & FeedbackLoopFlag_ReadAndWriteDS));
	...
	if (!InRenderPass())
	{
		...
		{
			// need to update descriptors to reflect the new layout
			if (feedback_loop & FeedbackLoopFlag_ReadAndWriteDS)
			{
				// NVIDIA drivers appear to return random garbage when sampling the RT via a feedback loop, if the load op for
				// the render pass is CLEAR. Using vkCmdClearAttachments() doesn't work, so we have to clear the image instead.
				...
				{
					m_dirty_flags |= (DIRTY_FLAG_TFX_TEXTURE_0 << TFX_TEXTURE_DS);
					vkDs->TransitionToLayout(GSTextureVK::Layout::FeedbackLoop);
				}
			}
			else if (feedback_loop & FeedbackLoopFlag_ReadDS)
			{
				if (vkDs->GetLayout() != GSTextureVK::Layout::FeedbackLoop)
				{
...
bool GSDeviceVK::CreatePipelineLayouts()
{
	...
	dslb.AddBinding(TFX_TEXTURE_DS,
		(m_features.texture_barrier && !UseFeedbackLoopLayout()) ? VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT :
		                                                           VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE,
		1, VK_SHADER_STAGE_FRAGMENT_BIT);
...
bool GSDeviceVK::ApplyTFXState(bool already_execed)
{
	...
	if (m_current_pipeline_layout != PipelineLayout::TFX)
	{
		...
		std::array<TFX_TEXTURES, 2> texture_types = { TFX_TEXTURE_RT, TFX_TEXTURE_DS };
	...
	if (flags & DIRTY_FLAG_TFX_TEXTURES)
	{
		...
		if (flags & DIRTY_FLAG_TFX_TEXTURE_DS)
		{
			if (m_features.texture_barrier && !UseFeedbackLoopLayout())
			{
				dsub.AddInputAttachmentDescriptorWrite(
					VK_NULL_HANDLE, TFX_TEXTURE_DS, m_tfx_textures[TFX_TEXTURE_DS]->GetView(), VK_IMAGE_LAYOUT_GENERAL);
			}
			else
			{
				dsub.AddImageDescriptorWrite(VK_NULL_HANDLE, TFX_TEXTURE_DS, m_tfx_textures[TFX_TEXTURE_DS]->GetView(),
					m_tfx_textures[TFX_TEXTURE_DS]->GetVkLayout());
			}
		}
...
void GSDeviceVK::RenderHW(GSHWDrawConfig& config)
{
	...
	if (InRenderPass())
	{
		if ((!pipe.IsRTFeedbackLoop() && config.tex == m_current_render_target) ||
			(!pipe.IsDSFeedbackLoop() && config.tex == m_current_depth_target))
			EndRenderPass();
	}
	...
	if (pipe.IsDSFeedbackLoop())
	{
		...
		// If this is the first draw to the target as a feedback loop, make sure we re-generate the texture descriptor.
		// Otherwise, we might have a previous descriptor left over, that has the RT in a different state.
		m_dirty_flags |= (skip_first_barrier ? static_cast<u32>(DIRTY_FLAG_TFX_TEXTURE_DS) : 0);
	}
	...
	if (BindDrawPipeline(pipe))
		SendHWDraw(config, draw_rt, pipe.IsDSFeedbackLoop() ? draw_ds : nullptr,
...
void GSDeviceVK::UpdateHWPipelineSelector(GSHWDrawConfig& config, PipelineSelector& pipe)
{
	...
	if (m_features.texture_barrier && (config.ps.IsFeedbackLoop() || config.require_one_barrier || config.require_full_barrier))
	{
		...
		pipe.feedback_loop_flags |= (pipe.ds && config.ps.IsFeedbackLoopDepth()) ? FeedbackLoopFlag_ReadAndWriteDS : FeedbackLoopFlag_None;
	}
	if (!(pipe.feedback_loop_flags & FeedbackLoopFlag_ReadAndWriteDS))
	{
		pipe.feedback_loop_flags |= (config.tex && config.tex == config.ds) ? FeedbackLoopFlag_ReadDS : FeedbackLoopFlag_None;
	}
	...
	bool one_barrier, bool full_barrier, bool skip_first_barrier)
{
	...
	{
		barrier[0] = GetColorBufferBarrier(draw_rt);
		if (draw_ds)
			barrier[1] = GetDepthStencilBufferBarrier(draw_ds);
	}
	const u32 barriers_per_draw = draw_ds ? 2 : 1;
