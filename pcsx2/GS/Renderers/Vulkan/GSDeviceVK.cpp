	...
	bool color_feedback_loop, bool depth_feedback_loop)
{
	return RenderPass(
		...
		color_feedback_loop, depth_feedback_loop);
}
...
VkRenderPass GSDeviceVK::GetVkRenderPass(const RenderPass& rp)
{
	...
	{
		VkMemoryBarrier2 barrier = {VK_STRUCTURE_TYPE_MEMORY_BARRIER_2_KHR};
		...
		rpb.SetColorFeedbackBarrier(barrier, feedback_dependency);
	...
	if (rp.GetDepthFeedbackLoop())
	{
		...
		barrier.dstStageMask = VK_PIPELINE_STAGE_2_FRAGMENT_SHADER_BIT;
		barrier.dstAccessMask = GetFeedbackLoopInputAccessBits();
		rpb.SetDepthFeedbackBarrier(barrier, feedback_dependency);
	}
	...
	const GSVector4i& dRect, u32 dLevel, Filter filter)
{
	...
	const VkImageBlit ib{
		.srcSubresource = {.aspectMask = aspect, .mipLevel = sLevel, .baseArrayLayer = 0u, .layerCount = 1u},
		.srcOffsets = {{.x = sRect.left, .y = sRect.top, .z = 0}, {.x = sRect.right, .y = sRect.bottom, .z = 1}},
		.dstSubresource = {.aspectMask = aspect, .mipLevel = dLevel, .baseArrayLayer = 0u, .layerCount = 1u},
		.dstOffsets = {{.x = dRect.left, .y = dRect.top, .z = 0}, {.x = dRect.right, .y = dRect.bottom, .z = 1}}};
...
void GSDeviceVK::ExecuteCommandBufferAndRestartPresent(bool wait_for_completion, const char* reason, ...)
{
	...
	rp.renderPass = GetVkRenderPass(GetRenderPass(
		swap_chain_texture->GetVkFormat(), VK_FORMAT_UNDEFINED, VK_ATTACHMENT_LOAD_OP_LOAD, VK_ATTACHMENT_STORE_OP_STORE));
	rp.framebuffer = fb;
...
void GSDeviceVK::BeginRenderPass(const RenderPass& rp, const GSVector4i& rect)
{
	...
	begin_info.renderPass = GetVkRenderPass(m_current_render_pass);
...
bool GSDeviceVK::BeginPresentRenderPass(const RenderPass& rp, const GSVector4i& rect, GSTextureVK* swap_chain)
{
	...
	VkRenderPassBeginInfo begin_info = {VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO};
	begin_info.renderPass = GetVkRenderPass(rp);
