...
GSTexture* GSDeviceVK::CreateSurface(GSTexture::Type type, int width, int height, int levels, GSTexture::Format format, GSTexture::ShaderAccess access)
{
	std::unique_ptr<GSTexture> tex = GSTextureVK::Create(type, format, access, width, height, levels);
	if (!tex)
	{
		...
		tex = GSTextureVK::Create(type, format, access, width, height, levels);
	}
	...
	GSTextureVK* dTex, const GSVector4i& dtex_rc, const GSVector4i& dst_rc, bool allow_discard)
{
	...
	if (dTex->IsDepthStencil())
	{
		if (load_op == VK_ATTACHMENT_LOAD_OP_CLEAR)
			...
			BeginRenderPass((load_op == VK_ATTACHMENT_LOAD_OP_DONT_CARE) ?
					m_utility_depth_render_pass_discard : m_utility_depth_render_pass_load,
				dtex_rc);
	}
...
{
	GSTexture::ShaderAccess null_access = m_features.rov ? GSTexture::ShaderAccess::ReadWrite : GSTexture::ShaderAccess::ReadOnly;
	m_null_texture = GSTextureVK::Create(GSTexture::Type::RenderTarget, GSTexture::Format::Color, null_access, 1, 1, 1);
	if (!m_null_texture)
		return false;
...
void GSDeviceVK::PSSetShaderRWResource(GSTexture* rt, GSTexture* ds, bool write_rt, bool write_ds)
{
	GSTextureVK* vkRt = static_cast<GSTextureVK*>(rt);
	GSTextureVK* vkDs = static_cast<GSTextureVK*>(ds);
