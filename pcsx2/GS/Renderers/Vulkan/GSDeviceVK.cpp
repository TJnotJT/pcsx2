...
void GSDeviceVK::RenderHW(GSHWDrawConfig& config)
{
			...
			(fb.ds && m_current_framebuffer.ds == fb.ds)))
	{
		...
		if (fb.rt && m_current_framebuffer.rt_feedback)
			pipe.rt_feedback = true;
		if (fb.ds && m_current_framebuffer.depth_feedback)
	...
	GSTextureVK* depth_for_barrier =
		(config.IsFeedbackLoopDepth(pipe.ps) && m_features.texture_barrier &&
			(config.require_full_barrier || config.require_one_barrier)) ?
	...
	fb.rt_feedback = pipe.rt_feedback;
	fb.depth_feedback = pipe.depth_feedback;
	...
	if (BindDrawPipeline(pipe))
		SendHWDraw(config, pipe.rt_feedback ? fb.rt : nullptr, pipe.depth_feedback ? depth_for_barrier : nullptr,
