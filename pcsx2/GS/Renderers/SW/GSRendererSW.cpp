...
GSTexture* GSRendererSW::GetOutput(int i, float& scale, int& y_offset)
{
	...
	if (g_gs_device->ResizeRenderTarget(&m_texture[index], w, h, false, false))
	{
		...
		{
			m_texture[index]->Save(GetDrawDumpPath("%05d_f%05lld_fr%d_%05x_%s.bmp", s_n, g_perfmon.GetFrame(), i, (int)curFramebuffer.Block(), GSUtil::GetPSMName(curFramebuffer.PSM)), GetRegressionBuffer());
		}
	}
...
void GSRendererSW::Draw()
{
	...
	if (GSConfig.ShouldDump(s_n, g_perfmon.GetFrame()))
	{
		...
		if (GSConfig.SaveRT)
		{
			...
			m_mem.SaveBMP(s, m_context->FRAME.Block(), m_context->FRAME.FBW, m_context->FRAME.PSM, r.z, r.w, 0, 0, GetRegressionBuffer());
		}
