...
GSDrawScanlineCodeGenerator::GSDrawScanlineCodeGenerator(u64 key, void* code, size_t maxsize)
	...
	, t2(r8) , t3(r9)
	, _m_local(r10)
	, _m_left(r14) // only used if step_size != vecints
#else
	, a0(rdi), a1(rsi)
	, a2(rdx), a3(rcx)
	, t0(r10), t1(r9)
...
void GSDrawScanlineCodeGenerator::Init()
{
	...
	if (step_size != vecints)
	{
		...
		movaps(xmm0, _rip_local(temp.cov));
		RollVec32(xmm0, rax);
		movaps(_rip_local(temp.cov), xmm0);
	}
...
void GSDrawScanlineCodeGenerator::Step()
{
	...
	else
	{
		...
		add(_m_left, step_size);

		// if ((left & (vlen - 1)) == 0)
		//   fza_offset += vlen / 4;

		test(_m_left, vecints - 1);
		jne("@f");
		add(t0, vecsize / 2);
		L("@@");
	}
	...
	if (step_size != vecints)
	{
		...
		if (!m_sel.zequal)
		{
			RollVec64(xmm7, _z, xmm0, step_size);
			movaps(_rip_local(temp.z0), xmm7);
		}
		RollVec32(_f, step_size);
		RollVec32(_s, step_size);
		RollVec32(_t, step_size);
		RollVec32(_q, step_size);
		RollVec32(_f_rb, step_size);
		RollVec32(_f_ga, step_size);
		RollVec32(_rb, step_size);
		RollVec32(_ga, step_size);
	}
}
...
void GSDrawScanlineCodeGenerator::Swap(const Xmm& r0, const Xmm& r1, const Xmm& tmp)
{
	movdqa(tmp, r0);
	movdqa(r0, r1);
	movdqa(r1, tmp);
...
void GSDrawScanlineCodeGenerator::Swap(const Ymm& r0, const Ymm& r1, const Ymm& tmp)
{
	movdqa(tmp, r0);
	movdqa(r0, r1);
	movdqa(r1, tmp);
...
void GSDrawScanlineCodeGenerator::RollVec32(const Xmm& dst, int roll)
{
	switch (roll)
	{
		case 0:
...
void GSDrawScanlineCodeGenerator::RollVec32(const Ymm& dst, const Ymm& tmp, int roll)
{
	switch (roll)
	{
		case 0:
...
void GSDrawScanlineCodeGenerator::RollVec64(const Xmm& dst0, const Xmm& dst1, const Xmm& tmp, int roll)
{
	switch (roll)
	{
		case 0:
...
void GSDrawScanlineCodeGenerator::RollVec64(const Ymm& dst0, const Ymm& dst1, const Ymm& tmp, int roll)
{
	switch (roll)
	{
		case 0:
...
#define ROLL_SWITCH(num, func, roll_reg, ...) \
	do                                        \
	{                                         \
		Label cases[4];                       \
		Label end;                            \
...
void GSDrawScanlineCodeGenerator::RollVec32(const Xmm& dst, const AddressReg& roll)
{
	ROLL_SWITCH(4, RollVec32, roll, dst);
}

void GSDrawScanlineCodeGenerator::RollVec64(const Xmm& dst0, const Xmm& dst1, const Xmm& tmp, const AddressReg& roll)
{
	ROLL_SWITCH(4, RollVec64, roll, dst0, dst1, tmp);
}

void GSDrawScanlineCodeGenerator::RollVec32(const Ymm& dst, const Ymm& tmp, const AddressReg& roll)
{
	ROLL_SWITCH(8, RollVec32, roll, dst, tmp);
}

void GSDrawScanlineCodeGenerator::RollVec64(const Ymm& dst0, const Ymm& dst1, const Ymm& tmp, const AddressReg& roll)
{
	ROLL_SWITCH(8, RollVec64, roll, dst0, dst1, tmp);
}