...
static float2 get_xy_unscaled(float2 xy)
{
	return round(xy / cb.vertex_scale) / 16.0f;
}

// Get the XY deltas in GS pixel coordinates, using first vertex as the origin.
static float2x2 get_xy_deltas_unscaled(const thread& MainVSOut v0, const thread& MainVSOut v1, const thread& MainVSOut v2)
{
...
float2 get_aa1_triangle_expand_dir(const thread MainVSOut& v0, const thread& MainVSOut v1, const thread& MainVSOut v2)
{
...
void extrapolate_aa1_triangle_edge(thread& VS_OUTPUT v0, VS_OUTPUT v1, VS_OUTPUT v2, const thread& float2x2 dp_mat, float2 dp)
{
	// Get texture deltas
	#if VS_TME
		#if VS_FST
	...
	#if VS_TME
		float2x2 dt_grad = dt * inv_dp_mat; // Texture
	#endif
	#if VS_IIP
		float2x4 dc_grad = dc * inv_dp_mat; // Color
	#endif
	float2 dz_grad = dz * inv_dp_mat; // Depth
	float2 df_grad = df * inv_dp_mat; // Fog
	float2 dq_grad = dq * inv_dp_mat; // Q
	...
	v0.p.z += dz_grad * dp; // Extrapolate depth

	v0.t.z += dot(df_grad, dp); // Extrapolate fog
}
	...
	switch (VS_EXPAND_TYPE)
	{
		...
		case VSExpand::TriangleAA1:
		{
			...
			else if (edge)
			{
				...
				MainVSOut out      = vs_main_run(load_vertex(vertices[indices[3 * prim_id + (is_bottom ? i1 : i0)]]), cb);
				MainVSOut other    = vs_main_run(load_vertex(vertices[indices[3 * prim_id + (is_bottom ? i0 : i1)]]), cb);
				MainVSOut opposite = vs_main_run(load_vertex(vertices[indices[3 * prim_id + i2]]), cb);

				float2x2 pos_deltas = get_xy_deltas_unscaled(out, other, opposite);
				...
				extrapolate_aa1_triangle_edge(out, other, opposite, pos_deltas, expand_dir);
			...
			else // Corner cap
			{
				...
				float2 edge_expand_dir_0 = get_aa1_triangle_expand_dir(out, other, opposite);
				float2 edge_expand_dir_1 = get_aa1_triangle_expand_dir(out, opposite, other);
