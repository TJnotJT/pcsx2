...
struct PSMain
{
	...
	{
		return sample_p(uint(cdenorm(u, 255.f)));
	}
	...
	uint4 sample_4_index(float4 uv)
	{
		...
		{
			i = uint4(cdenorm(c, 128.f)); // Denormalize value
		}
		else
		{
			i = uint4(cdenorm(c, 255.f)); // Denormalize value
		}
	...
	float4 sample_depth(float2 st)
	{
		...
		if (PS_AEM_FMT == FMT_24)
			...
			t = cdenorm(sample_4p(uint4(t.aaaa))[0], 255.f);
	...
	float4 fetch_gXbY()
	{
		...
		{
			uint4 rt = uint4(cdenorm(fetch_raw_color(), 255.f));
	...
	float4 sample_color(float2 st)
	{
		...
		for (int i = 0; i < 4; i++)
		{
			...
			if (PS_AEM_FMT == FMT_24)
				...
				c[i].a = c[i].a >= 0.5 ? cb.ta.y : !PS_AEM || any((int3(cdenorm(c[i].rgb, 255.f)) & 0xF8) != 0) ? cb.ta.x : 0.f;
		}
		...
		if (PS_AEM_FMT == FMT_32 && PS_PAL_FMT == 0 && PS_RTA_SRC_CORRECTION)
			...
			t.a = cdenorm(t.a, 255.f);
		
		t.rgb = cdenorm(t.rgb, 255.f);
	...
	{
		if (PS_FOG)
			C.rgb = trunc(mix(cb.fog_color, C.rgb, cdenorm(f, 255.f) / 256.f));
	}
	...
	void ps_blend(thread float4& Color, thread float4& As_rgba)
	{
		...
		if (SW_BLEND)
		{
			...
			float multi = PS_COLCLIP_HW ? 65535.f : 255.f;
			float3 Cd = cdenorm(current_color.rgb, multi);
			float3 Cs = Color.rgb;
			...
			if (PS_BLEND_HW == 1)
			{
				...
				// if c.rgb > 255.f then we further need to adjust alpha accordingly,
	...
	MainResult ps_main()
	{
		...
		if (PS_SHUFFLE)
		{
			...
			{
				uint4 denorm_c = uint4(C);
				uint2 denorm_TA = uint2(cdenorm(cb.ta, 255.f));
