...
fragment uint ps_copy_uint(ConvertShaderData data [[stage_in]], ConvertPSRes<uint> res)
{
	return res.sample(data.t);
}

fragment ushort ps_convert_rgba8_16bits(ConvertShaderData data [[stage_in]], ConvertPSRes<float> res)
{
...
fragment uint ps_convert_float32_uint32(ConvertShaderData data [[stage_in]], ConvertPSDepthRes res)
{
	return uint(0x1p32 * res.sample(data.t));
}

fragment uint ps_convert_float32_uint24(ConvertShaderData data [[stage_in]], ConvertPSDepthRes res)
{
	return uint(0x1p32 * res.sample(data.t)) & 0xFFFFFF;
}
...
{
	float3 c = float3(unorm.rgb * 255.5f);
	return c.r | (c.g << 8) | (c.b << 16);
}

static uint rgba8_to_uint16(float4 unorm)
{
	uvec2 c = uvec2(unorm.rg * 255.5f);
	return c.r | (c.g << 8);
}
...
{
	return res.sample(data.t) & 0xFFFFFFu;
}
...
fragment DepthOut ps_convert_uint32_float32(ConvertShaderData data [[stage_in]], ConvertPSRes<uint> res)
{
	return exp2(-32.0f) * res.sample(data.t).r;
}

fragment DepthOut ps_convert_uint32_float24(ConvertShaderData data [[stage_in]], ConvertPSRes<uint> res)
{
	return exp2(-32.0f) * (res.sample(data.t).r & 0xFFFFFF);
}

fragment DepthOut ps_convert_float32_color_to_depth(ConvertShaderData data [[stage_in]], ConvertPSRes<float> res)
{
