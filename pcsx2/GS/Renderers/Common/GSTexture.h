...
protected:
	...
	ShaderAccess m_shader_access = ShaderAccess::ReadOnly;
...
public:
	...
	__fi ShaderAccess GetShaderAccess() const { return m_shader_access; }
	...
	static bool IsShaderReadWriteFormat(Format format);
	...
	{
		return (type >= Type::RenderTarget && type <= Type::DepthStencil);
	}
	static __fi bool IsRenderTarget(Type type)
	{
		return (type == Type::RenderTarget);
	}
	static __fi bool IsDepthStencil(Type type)
	{
		return (type == Type::DepthStencil);
	}
	static __fi bool IsDepthColor(Type type, Format format)
	{
		return (type == Type::RenderTarget) && (format == Format::DepthColor);
	}
	...
	{
		return IsDepthStencil(type) || IsDepthColor(type, format);
	}
	static __fi bool IsShaderReadOnly(ShaderAccess access)
	{
		return access == ShaderAccess::ReadOnly;
	}
	static __fi bool IsShaderReadWrite(ShaderAccess access)
	{
		return access == ShaderAccess::ReadWrite;
	}
	...
	{
		return IsDepthColor(m_type, m_format);
	}
	...
	__fi bool IsShaderReadOnly() const
	{
		return IsShaderReadOnly(m_shader_access);
	}
	__fi bool IsShaderReadWrite() const
	{
		return IsShaderReadWrite(m_shader_access);
	}

	virtual bool IsShaderReadWriteMode() const
	{
		// Backends that track state explicitly can specialize this to use the actual state.
		return IsShaderReadWrite();
	}
