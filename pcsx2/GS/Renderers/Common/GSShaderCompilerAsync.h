...
public:
	...
	void SetDone() { m_done.store(true, std::memory_order_release); }
...
private:
	JobType m_job_type;
	std::atomic<bool> m_done = false; // Done flag set when job is entered into cache (not just done compiling).
