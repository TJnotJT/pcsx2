...
#pragma pack(push, 4)
	struct PSSelector
	{
		...
		__fi bool IsFeedbackLoopDS() const
		{
			// Note: currently manual depth testing/interpolation for accurate prims is bundled with zclamp to reduce pipeline combinations.
			// The zclamp is used to indicate that either Z write of Z testing is enabled.
			return accurate_prims && zclamp;
		}
