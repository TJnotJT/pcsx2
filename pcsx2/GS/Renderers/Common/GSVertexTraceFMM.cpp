...
class CURRENT_ISA::GSVertexTraceFMM
{
	...
	static void FindMinMax(GSVertexTrace& vt, const void* vertex, int v_count, const u16* index, int count);
...
template <GS_PRIM_CLASS primclass, u32 iip, u32 tme, u32 fst, u32 color, bool flat_swapped>
void GSVertexTraceFMM::FindMinMax(GSVertexTrace& vt, const void* vertex, int v_count, const u16* index, int count)
{
	...
	if (iip) // Gourad shading; no need for indexing
	{
		for (int i = 0; i < v_count - 1; i += 2) // 2x loop unroll
		{
			ProcessVertices(v[i], v[i + 1]);
		}
		if (v_count & 1)
		{
			ProcessVertices(v[v_count - 1], v[v_count - 1]);
		}
	}
	else if (primclass == GS_TRIANGLE_CLASS)
	{
		...
		if (i_count & 1)
		{
			ProcessVertices(v[index[i_count - 3]], v[index[i_count - 2]], !flat_swapped);
			ProcessVertices(v[index[i_count - 1]], v[index[i_count - 2]], flat_swapped);
		}
	}
	else
	{
		// Everything else. Process vertices sequentially two at a time.
		// Color control for lines/sprites is handled in ProcessVertices.
		for (int i = 0; i < i_count - 1; i += 2) // 2x loop unroll
		{
			ProcessVertices(v[index[i + 0]], v[index[i + 1]]);
		}
		if (count & 1)
		{
			ProcessVertices(v[index[i_count - 1]], v[index[i_count - 1]]);
		}
	}

	GSVector4 o(context->XYOFFSET);
	GSVector4 s(1.0f / 16, 1.0f / 16, 2.0f, 1.0f); // TODO: Why is Z multiplied by 2?

	vt.m_min.p = (GSVector4(pmin) - o) * s;
	vt.m_max.p = (GSVector4(pmax) - o) * s;

	// FIXME: We can fully determin m_eq here without having to correct depth trace, etc.
