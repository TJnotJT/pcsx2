...
class CURRENT_ISA::GSVertexTraceFMM
{
	...
	static void FindMinMax(GSVertexTrace& vt, const void* vertex, int v_count, const u16* index, int i_count);
...
template <GS_PRIM_CLASS primclass, u32 iip, u32 tme, u32 fst, u32 color, bool flat_swapped>
void GSVertexTraceFMM::FindMinMax(GSVertexTrace& vt, const void* vertex, int v_count, const u16* index, int i_count)
{
	const GSDrawingContext* context = vt.m_state->m_context;

	int n = GSUtil::GetClassVertexCount(primclass);
	...
	if (iip) // Gourad shading; treat all vertices the same.
	{
		...
		if (v_count & 1)
		{
			ProcessVertices(v[index[v_count - 1]], v[index[v_count - 1]]);
		}
	}
	else if (primclass == GS_TRIANGLE_CLASS)
	{
		...
		{
			ProcessVertices(v[index[i + 0]], v[index[i + 3]], iip && !flat_swapped);
