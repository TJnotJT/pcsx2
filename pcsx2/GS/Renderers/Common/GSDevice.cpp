...
// Callback for iterating over selector bits
using PipelineFieldAction = std::function<void(
	const GSHWDrawConfig::PipelineSelectorFieldDesc& field_desc,
	u32 selector_num,
	u32 start_bit, u32 end_bit, u32 mask,
	u32 start_bit_2, u32 end_bit_2, u32 mask_2)>;
...
const std::vector<GSHWDrawConfig::ShaderDefine>& GSHWDrawConfig::GetUberShaderPSSelectorDefines()
{
	...
	if (defines.empty())
	{
		defines = GetUberShaderSelectorDefines<std::size(ps_selector_fields)>(
			ps_selector_fields, GSHWDrawConfig::ShaderPushConstants::PS_UBER_SELECTOR);
	}
	return defines;
}
...
const std::vector<GSHWDrawConfig::ShaderDefine>& GSHWDrawConfig::GetUberShaderVSSelectorDefines()
{
	...
	if (defines.empty())
	{
		defines = GetUberShaderSelectorDefines<std::size(vs_selector_fields)>(
			vs_selector_fields, GSHWDrawConfig::ShaderPushConstants::VS_UBER_SELECTOR);
	}
	return defines;
}
...
{
	::GetUberShaderSelector<VSSelector, std::size(vs_selector_fields)>(
		vs, vs_selector_fields, ShaderPushConstants::NUM_UBER_SELECTORS,
		ShaderPushConstants::VS_UBER_SELECTOR, pc_out.uber_selectors);

	::GetUberShaderSelector<PSSelector, std::size(ps_selector_fields)>(
		ps, ps_selector_fields, ShaderPushConstants::NUM_UBER_SELECTORS,
...
std::pair<GSHWDrawConfig::PSSelector, bool> GSHWDrawConfig::GetNthUberPSSelector(u32 n)
{
	...
	bool valid = true;
	valid &= (rov_depth > static_cast<u32>(PS_ROV_DEPTH::READ_WRITE));
	valid &= !sel.HasColorOutput() || sel.HasColorROV(); // Only allow color ROV output.
	valid &= !sel.HasDepthOutput() || sel.HasDepthROV(); // Only allow depth ROV output.
	valid &= sel.HasColorROV() || sel.HasDepthROV(); // Must have color or depth ROV.

	return std::make_pair(sel, valid);
}

std::pair<GSHWDrawConfig::VSSelector, bool> GSHWDrawConfig::GetNthUberVSSelector(u32 n)
{
	VSSelector sel{};
	sel.uber_enable = true;
	return std::make_pair(sel, true);
}
