...
GSTexture* GSDevice::FetchSurface(GSTexture::Usage usage, GSVector2i size, int levels, GSTexture::Format format, bool clear,
	bool prefer_unused_texture)
{
	return FetchSurface(usage, size.x, size.y, levels, format, clear, prefer_unused_texture);
}

GSTexture* GSDevice::FetchSurface(GSTexture::Usage usage, int width, int height, int levels, GSTexture::Format format, bool clear,
	bool prefer_unused_texture)
{
...
GSTexture* GSDevice::CreateFeedbackDepth(int w, int h, bool clear, bool prefer_reuse)
{
	return FetchSurface(GSTexture::FeedbackDepth, w, h, 1, GSTexture::Format::DepthStencil, clear, !prefer_reuse);
}

GSTexture* GSDevice::CreateFeedbackDepth(const GSVector2i& size, bool clear, bool prefer_reuse)
{
	return FetchSurface(GSTexture::FeedbackDepth, size.x, size.y, 1, GSTexture::Format::DepthStencil, clear, !prefer_reuse);
}
...
GSTexture* GSDevice::CreateShaderWriteTexture(int w, int h, GSTexture::Format format, bool clear, bool prefer_reuse)
{
	return FetchSurface(GSTexture::ShaderWriteTexture, w, h, 1, format, clear, !prefer_reuse);
}

GSTexture* GSDevice::CreateShaderWriteTexture(const GSVector2i& size, GSTexture::Format format, bool clear, bool prefer_reuse)
{
	return FetchSurface(GSTexture::ShaderWriteTexture, size.x, size.y, 1, format, clear, !prefer_reuse);
}
...
void GSDevice::CAS(GSTexture*& tex, GSVector4i& src_rect, GSVector4& src_uv, const GSVector4& draw_rect, bool sharpen_only)
{
	...
	{
		delete m_cas;
		m_cas = CreateShaderWriteTexture(dst_width, dst_height, GSTexture::Format::Color, true, true);
		if (!m_cas)
		{
