...
{
	// Only these usage combinations are allowed
	if (!(usage == Texture || usage == RenderTarget || usage == DepthStencil ||
		usage == FeedbackTarget || usage == FeedbackDepth ||
		usage == ShaderWriteTarget || usage == ShaderWriteTexture))
	{
		return false;
	}
