...
bool GSTexture::Save(const std::string& fn)
{
	...
	if (m_format == Format::DepthStencil || m_format == Format::Float32 || IsIntegerFormat())
	{
		...
		g_gs_device->StretchRect(this, GSVector4::cxpr(0.0f, 0.0f, 1.0f, 1.0f), temp, GSVector4(GetRect()),
			IsIntegerFormat() ? ShaderConvert::UINT32_TO_RGBA8 : ShaderConvert::FLOAT32_TO_RGBA8, false);
