...
GIFRegTEX0 GSDrawingContext::GetSizeFixedTEX0(GSVector4 uvf, bool linear, bool mipmap) const
{
	if (mipmap)
	...
	const GSVector4i uv = GSVector4i(uvf.floor());

	// Add + 1 to the max U or V to make the min width/height exclusive
	const int min_width = GetMaxUV(TW, WMS, MINU, MAXU, uv.left, uv.right) + 1;
	const int min_height = GetMaxUV(TH, WMT, MINV, MAXV, uv.top, uv.bottom) + 1;
	...
	if (TEX0.TW != res.TW || TEX0.TH != res.TH)
	{
		GL_DBG("FixedTEX0 %05x %d %d tw %d=>%d th %d=>%d st (%.0f,%.0f,%.0f,%.0f) uvmax %d,%d wm %d,%d (%d,%d,%d,%d)",
			...
			uvf.left, uvf.top, uvf.right, uvf.bottom,
