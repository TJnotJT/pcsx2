...
template <u32 tme, u32 fst>
bool GSUtil::AreTrianglesQuad(const GSVertex* RESTRICT vin, const u16* index0, const u16* index1,
	TriangleOrdering* out_triangle0, TriangleOrdering* out_triangle1)
{
...
// Checks if the given rectangles can be arranged in a grid.
// Each rectangle on the grid must have the same size and the
// stride between adjacent columns/rows of the rectangles must
// be the same. We cannot have overlapping rectangles.
// The arguments are all modifiable though the list of index/rectangle
// pairs is the only input. If successful, return the grid size
