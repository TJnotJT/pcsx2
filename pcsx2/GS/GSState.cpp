...
void GSState::DumpVertices(const std::string& filename)
{
	...
	constexpr int VERT_INDEX_WIDTH = 8;
	...
	auto WriteVertexIndex = [this, &file](int index) {
		const std::string quotedIndex = QUOTE + std::to_string(index) + QUOTE;
		file << std::left << std::dec << std::setw(VERT_INDEX_WIDTH) << std::setfill(' ') << quotedIndex;
	};
...
void GSState::DumpTransferImages()
{
	...
	for (int i = 0; i < static_cast<int>(m_draw_transfers.size()); ++i)
	{
		...
		if (transfer.ee_to_gs)
		{
			// Transferring GS->GS then only the destination info is relevant.
			filename = GetDrawDumpPath("%05d_xfer%02d_EE_to_GS_%03x_%d_%s_%d_%d_%d_%d.png",
		...
		else
		{
			...
			filename = GetDrawDumpPath("%05d_xfer%02d_EE_to_GS_%03x_%d_%s_%03x_%d_%s_%d_%d_%d_%d.bmp",
