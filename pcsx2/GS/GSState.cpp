...
bool GSState::TrianglesAreQuads2()
{
	...
	for (u32 idx = 0; idx < m_index.tail; idx += 6)
	{
		...
		int m = GSVector4::cast((bbox - offset) == prev_bbox).mask();
		int halign = (m & 0b0101) == 0b0101;
		int valign = (m & 0b1010) == 0b1010;
		int hadj = GSVector4::cast(bbox.xzxz() == prev_bbox.zxzx()).mask() & 3;
		int vadj = GSVector4::cast(bbox.ywyw() == prev_bbox.ywyw()).mask() & 3;

		bool adjacent = (halign && (hadj == 1 || hadj == 2)) || (valign && (vadj == 1 || vadj == 2));
