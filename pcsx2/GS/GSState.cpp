...
//	u8 uses_border = 0;
//
//	// TODO: Replace this with a flag that says "huge or nans" detected in texture coords.
...
GSState::TextureMinMaxResult GSState::GetTextureMinMax(GIFRegTEX0 TEX0, GIFRegCLAMP CLAMP, bool linear, bool expand_region_mode)
{
	const int TW = TEX0.TW;
	const int TH = TEX0.TH;
	...
	// Contains all possible values that may be samples from
	GSVector4i full_rect(0, 0, width - 1, height - 1);

	// Software renderer fixes TEX0 so that TW/TH contain MAXU/MAXV so no need to expand.
	// Hardware renderer doesn't, and handles it in the texture cache, so we may need to
	// expand full_rect past the limits of TW/TH if region repeat or region clamp allows it.
	if (expand_region_mode)
	{
		GSVector4i full_rect_region(0, 0, 0, 0);

		auto GetRegionBound = [](int WM, int MIN, int MAX, int* min_out, int* max_out) {
			switch (WM)
			{
				case CLAMP_REGION_CLAMP:
					*min_out = MIN;
				...
				case CLAMP_REGION_REPEAT:
					...
					*max_out = MIN | MAX;
					break;
				default:
					pxFail("Invalid clamp/wrap mode"); // Impossible
			}
		};

		if (WMS == CLAMP_REGION_CLAMP || WMS == CLAMP_REGION_REPEAT)
			GetRegionBound(WMS, MINU, MAXU, &full_rect_region.left, &full_rect_region.right);
		if (WMT == CLAMP_REGION_CLAMP || WMT == CLAMP_REGION_REPEAT)
			GetRegionBound(WMT, MINV, MAXV, &full_rect_region.top, &full_rect_region.bottom);

		full_rect = full_rect.runion(full_rect_region);
	}

	GSVector4i vertex_rect; // Rectangle computed from UV bounds of vertices
	u8 uses_border = 0;
		...
		m_vt.m_max.t.y >= FLT_MAX || m_vt.m_min.t.y <= -FLT_MAX)
	{
		// If any of the min/max values are +-FLT_MAX we can't rely on them
		// so just assume full texture.
		vertex_rect = full_rect;
		uses_border = 0xF;
	}
	else
	{
		// Calculate an approximation of the actual UV range that will be used in the draw
		// to avoid having to reload the whole texture.
		GSVector4 vertex_rect_f = m_vt.m_min.t.xyxy(m_vt.m_max.t);
		
		if (linear)
		{
			vertex_rect_f += GSVector4(-0.5f, -0.5f, 0.5f, 0.5f);
		}

		vertex_rect = GSVector4i(vertex_rect_f.floor());

		bool min_boundary_u, min_boundary_v, max_boundary_u, max_boundary_v;

		GetClampWrapMinMaxUV(TW, WMS, MINU, MAXU,
			vertex_rect.left, vertex_rect.right, &vertex_rect.left, &vertex_rect.right,
			&min_boundary_u, &max_boundary_u);

		GetClampWrapMinMaxUV(TH, WMT, MINV, MAXV,
			vertex_rect.top, vertex_rect.bottom, &vertex_rect.top, &vertex_rect.bottom,
			&min_boundary_v, &max_boundary_v);
	...
	GSVector4i final_rect = full_rect.rintersect(vertex_rect);

	// Add +1 to bottom/right to make exclusive
	const int empty = (final_rect + GSVector4i(0, 0, 1, 1)).rempty();

	if (expand_region_mode && empty)
		pxFail("Impossible");
	
	if (!expand_region_mode && empty)
	{
		// Something strange happened, the region repeat/clamp mode
		// caused vertices to go outside the texture bounds.
		// Just assume that full texture is used.
		final_rect = full_rect;
		uses_border = 0xF;
	}

	// Add +1 to bottom/right to make exclusive since this is the usual convention for rectangles
	final_rect += GSVector4i(0, 0, 1, 1);
	
	return {final_rect, uses_border};
}
...
void GSState::GetTextureMinMaxAxisAlignedHelper(
	GIFRegTEX0 TEX0, u32 fst, bool linear, const GSVertex* vertex, u16 index0, u16 index1, GSVector4* minmax)
{
	...
	float x1 = (float)(vertex[index[ix1]].XYZ.X - m_xyof.z) / 16.0f;
	float y1 = (float)(vertex[index[iy1]].XYZ.Y - m_xyof.w) / 16.0f;
	...
	float u0_interp = ((x1 - std::ceilf(x0)) * u0 + (std::ceilf(x0) - x0) * u1) / (x1 - x0);
	float v0_interp = ((y1 - std::ceilf(y0)) * v0 + (std::ceilf(y0) - y0) * v1) / (y1 - y0);
	float u1_interp = ((x1 - std::floorf(x1)) * u0 + (std::floorf(x1) - x0) * u1) / (x1 - x0);
	float v1_interp = ((y1 - std::floorf(y1)) * v0 + (std::floorf(y1) - y0) * v1) / (y1 - y0);
...
bool GSState::GetTextureMinMaxAxisAligned(GIFRegTEX0 TEX0, GIFRegCLAMP CLAMP, bool linear, GSState::TextureMinMaxResult* result)
{
	...
	if (primclass == GS_PRIM_CLASS::GS_TRIANGLE_CLASS)
	{
		for (u32 i = 0; i < m_index.tail; i += 3)
		{
			...
			if (!IsAxisAlignedRightTriangle(vertex, &idx[0], fst, &i_tri[0]))
				return false;
			GetTextureMinMaxAxisAlignedHelper(TEX0, fst, linear, vertex, idx[i_tri[1]], idx[i_tri[2]], &minmax);
		}
	}
	else if (primclass == GS_PRIM_CLASS::GS_SPRITE_CLASS)
	{
		...
		{
			GetTextureMinMaxAxisAlignedHelper(TEX0, fst, linear, vertex, index[i], index[i + 1], &minmax);
		}
	}
	else
	{
