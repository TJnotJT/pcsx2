...
bool GSState::DeindexVertices()
{
	...
	if (idx_buff.tail > UINT16_MAX) [[unlikely]]
	{
		Console.Warning("Error: Cannot deindex %u (> %u) vertices.", idx_buff.tail, UINT16_MAX);
		return false;
	}
...
bool GSState::GetVertexUVRoundingInfoImpl(const bool upscaling, VertexUVRoundingInfo* info)
{
	...
	// ShaderSpriteAlign will likely cause graphical issues.
	const bool use_uv_rounding =
		(!upscaling && GSConfig.AccurateUVRounding) ||
		(upscaling && GSConfig.ShaderSpriteAlign != GSShaderSpriteAlignMode::Off);
	...
	// Don't do rounding with AA1, it's likely unnecessary and the edge pixels may break things.
	...
	for (u32 i = 0; i < count; i += n)
	{
		...
		if constexpr (tme)
		{
			...
			for (u32 j = 0; j < n; j++)
			{
				...
				{
					// Decide if we will allow pixel shader rounding adjustments
					// (otherwise only do vertex shader adjustments).
					round_U |= (allow_round_U ? ROUND_UV_PER_PIXEL : 0);
					round_V |= (allow_round_V ? ROUND_UV_PER_PIXEL : 0);
				}
				else
				{
					// SW renderer only had per-pixel adjustments, so either do rounding or not at all.
					 if (!allow_round_U)
