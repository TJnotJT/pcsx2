...
GSState::PRIM_OVERLAP GSState::BatchPrimsNoOverlap(bool save_drawlist, bool save_bbox)
{
	...
	while (i < count)
	{
		...
		while (j < count)
		{
			...
			if (primclass == GS_TRIANGLE_CLASS && j > i && (all == prim).mask() == 0xffff)
			{
				// If this triangle forms a quad with the previous one then there is no overlap.
				GSUtil::TriangleOrdering tri0;
				GSUtil::TriangleOrdering tri1;

				if (!GSUtil::AreTrianglesQuad<0, 0>(m_vertex.buff, &index[j - 3], &index[j], &tri0, &tri1))
				{
