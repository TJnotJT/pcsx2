...
bool do_dump = false;
bool use_hack = true;
bool init_range_hack = false;
std::vector<Point> pointsHack;
std::vector<Point> pointsHackD;
...
std::tuple<double, double> InterpolateEdgeFunctionsUV(double e01, double e12, double e20, double U0, double V0, double U1, double V1, double U2, double V2)
{
	return {(e12 * U0 + e20 * U1 + e01 * U2) / (e01 + e12 + e20), (e12 * V0 + e20 * V1 + e01 * V2) / (e01 + e12 + e20)};
}

bool CheckXYBounds(double x, double y, int minX, int minY, int maxX, int maxY, bool dxBigger, bool dyBigger)
{
	if (dxBigger)
	{
...
std::tuple<Point, Point, Point> SortPoints(Point v0, Point v1, Point v2)
{
...
// Order the points so that v0-v1 is a top or right edge, v1-v2 is a right or bottom edge, and v0-v2 is a left edge.
std::tuple<Point, Point, Point, EdgeType, EdgeType, EdgeType> SortAndClassifyTriangleVerts(Point v0, Point v1, Point v2)
{
	std::tie(v0, v1, v2) = SortPoints(v0, v1, v2);
	EdgeType t01 = (v0.y == v1.y) ? EdgeType::TOP : EdgeType::RIGHT;
	EdgeType t12 = (v1.y < v2.y) ? EdgeType::RIGHT : (v1.y > v2.y ? EdgeType::LEFT : EdgeType::BOTTOM);
...
void CalculateUVHelper(int U, int V, int W, int H, GIFRegCLAMP clamp, int& minU, int& minV, int& maxU, int& maxV)
{
	...
	if (save_points)
	{
		pointsHack[pointsHack.size() - 1].U = (double)U;
		pointsHack[pointsHack.size() - 1].V = (double)V;
	}
...
// Calculate UV coordinates for a point based on edge functions and adjust the min/max UV values accordingly
// W and H are the texture width and height
// minU, minV, maxU, maxV are the current min/max UV values and output
void CalculateUV_Slow(double e01, double e12, double e20, EdgeType t0, EdgeType t1, EdgeType t2,
...
{
	auto [U, V] = InterpolateEdgeFunctionsUV(e01, e12, e20, U0, V0, U1, V1, U2, V2);
	Point temp;
	if (save_points)
	{
		temp = pointsHackD[pointsHackD.size() - 1];
		pointsHackD.pop_back();
		pointsHack.pop_back();
	}
	if (bilinear)
	{
		for (double dU : {-0.5, 0.5}) {
			for (double dV : {-0.5, 0.5}) {
				if (save_points)
				{
					pointsHackD.push_back(Point(temp.x, temp.y, U + dU, V + dV));
					pointsHack.push_back(Point(temp.x, temp.y, 0, 0));
				}
				CalculateUVHelper(static_cast<int>(std::floor(U + dU)), static_cast<int>(std::floor(V + dV)), W, H, clamp, minU, minV, maxU, maxV);
			}
		}
	}
	else
	{
		if (save_points)
		{
			pointsHackD.push_back(Point(temp.x, temp.y, U, V));
			pointsHack.push_back(Point(temp.x, temp.y, 0, 0));
		}
		CalculateUVHelper(static_cast<int>(std::floor(U)), static_cast<int>(std::floor(V)), W, H, clamp, minU, minV, maxU, maxV);
	}
}


void CalculateUV(double e01, double e12, double e20, EdgeType t0, EdgeType t1, EdgeType t2,
	double U0, double V0, double U1, double V1, double U2, double V2,
	int W, int H, bool bilinear, const GIFRegCLAMP& clamp, int& minU, int& minV, int& maxU, int& maxV)
{
	auto [U, V] = InterpolateEdgeFunctionsUV(e01, e12, e20, U0, V0, U1, V1, U2, V2);
...
bool CheckScissor(int x, int y, int SCAX0, int SCAY0, int SCAX1, int SCAY1)
{
...
void CheckScissorUVs(Point v0, Point v1, Point v2, EdgeType t01, EdgeType t12, EdgeType t20, int W, int H, bool bilinear,
	...
	for (int x : {SCAX0, SCAX1})
	{
		for (int y : {SCAY0, SCAY1})
		{
			...
			if (CheckEdgeFunction(e01, t01) && CheckEdgeFunction(e12, t12) && CheckEdgeFunction(e20, t20))
			{
				if (save_points)
				{
					pointsHack.push_back(Point(x, y, 0, 0));
					pointsHackD.push_back(Point(x, y, 0, 0));
				}
				CalculateUV(e01, e12, e20, t01, t12, t20, v0.U, v0.V, v1.U, v1.V, v2.U, v2.V, W, H, bilinear, clamp, minU, minV, maxU, maxV);
			}
		}
	}
}

//// Rasterize the boundary of the triangle and output min/max UVs
//void EdgeWalkTriangleMinMaxUVImpl(Point v0, Point v1, Point v2, EdgeType t01, EdgeType t12, EdgeType t20, int W, int H, bool bilinear,
...
//		if (dx > 0.0)
//		{
//			x = static_cast<int>(std::floor(v0.x));
//			y = static_cast<int>(std::ceil(v0.y));
//			sy = 1;
//		}
//		else
//		{
//			x = static_cast<int>(std::ceil(v0.x));
//			y = static_cast<int>(std::floor(v0.y));
//			sy = -1;
//		}
//	}
//	else if (dx == 0.0)
...
//	{
//		if (E01.a * std::ceil(v0.x) + E01.b * std::ceil(v0.y) + E01.c >= 0.0)
//		{
//			x = static_cast<int>(std::ceil(v0.x));
//			y = static_cast<int>(std::ceil(v0.y));
//		}
//		else if (E01.a * std::floor(v0.x) + E01.b * std::ceil(v0.y) + E01.c >= 0.0)
//		{
//			x = static_cast<int>(std::floor(v0.x));
//			y = static_cast<int>(std::ceil(v0.y));
//		}
...
//	else if ((dx < 0.0) && (dy > 0.0))
//	{
//		if (E01.a * std::floor(v0.x) + E01.b * std::ceil(v0.y) + E01.c >= 0)
//		{
//			x = static_cast<int>(std::floor(v0.x));
//			y = static_cast<int>(std::ceil(v0.y));
//		}
//		else if (E01.a * std::floor(v0.x) + E01.b * std::floor(v0.y) + E01.c >= 0)
//		{
//			x = static_cast<int>(std::floor(v0.x));
//			y = static_cast<int>(std::floor(v0.y));
//		}
...
//	else if ((dx < 0.0) && (dy < 0.0))
//	{
//		if (E01.a * std::floor(v0.x) + E01.b * std::floor(v0.y) + E01.c >= 0)
//		{
//			x = static_cast<int>(std::floor(v0.x));
//			y = static_cast<int>(std::floor(v0.y));
//		}
//		else if (E01.a * std::ceil(v0.x) + E01.b * std::floor(v0.y) + E01.c >= 0)
//		{
//			x = static_cast<int>(std::ceil(v0.x));
//			y = static_cast<int>(std::floor(v0.y));
//		}
...
//	else if ((dx > 0) && (dy < 0))
//	{
//		if (E01.a * std::ceil(v0.x) + E01.b * std::floor(v0.y) + E01.c >= 0)
//		{
//			x = static_cast<int>(std::ceil(v0.x));
//			y = static_cast<int>(std::floor(v0.y));
//		}
//		else if (E01.a * std::ceil(v0.x) + E01.b * std::ceil(v0.y) + E01.c >= 0)
//		{
//			x = static_cast<int>(std::ceil(v0.x));
//			y = static_cast<int>(std::ceil(v0.y));
//		}
...
//	const int minX = static_cast<int>(std::floor(std::min(v0.x, v1.x)));
//	const int minY = static_cast<int>(std::floor(std::min(v0.y, v1.y)));
//	const int maxX = static_cast<int>(std::ceil(std::max(v0.x, v1.x)));
//	const int maxY = static_cast<int>(std::ceil(std::max(v0.y, v1.y)));
...
//				if (!WALK_FULL_EDGE)
//				{
//					break; // Break after the first rasterizable point. This might not always be the "most ouside" point but should be much faster.
//				}
//			}
...
//		while (CheckXYBounds(x, y, minX, minY, maxX, maxY, dxBigger, dyBigger))
//		{
//			// Update min/max UV values if the current point is inside the triangle and XY is inside the scissor
//			if (CheckEdgeFunction(e01, t01) && CheckEdgeFunction(e12, t12) && CheckEdgeFunction(e20, t20) && CheckScissor(x, y, SCAX0, SCAY0, SCAX1, SCAY1))
//			{
	...
	const GIFRegSCISSOR& scissor, const GIFRegCLAMP& clamp, bool switchOrient, int& minU, int& minV, int& maxU, int& maxV)
{
	...
	// Initialize starting point by rounding correctly
	// Use right-hand-interior rule and brute force case analysis to get the correct starting point
	// FIXME: THIS IS WRONG!!! THERE CAN REALLY BE 3 CANDIDDATE POINTS, DRAW IT!
	int x, y;
	if (dy == 0.0)
	{
		if (dx > 0.0)
...
void EdgeWalkTriangleMinMaxUV(Point v0, Point v1, Point v2, int W, int H, bool bilinear, GIFRegSCISSOR scissor, GIFRegCLAMP clamp,
	int& minU, int& minV, int& maxU, int& maxV)
{
	// Rearrange the points in the correct order and walk along all edges
	auto [v0_, v1_, v2_, t01, t12, t20] = SortAndClassifyTriangleVerts(v0, v1, v2);

	// Check is the corners of the scissor region are inside the triangle
	CheckScissorUVs(v0_, v1_, v2_, t01, t12, t20, W, H, bilinear, scissor, clamp, minU, minV, maxU, maxV);

	// Walk along edges in the clockwise direction (if Y-axis points down)
	EdgeWalkTriangleMinMaxUVImpl(v0_, v1_, v2_, t01, t12, t20, W, H, bilinear, scissor, clamp, false, minU, minV, maxU, maxV);
	EdgeWalkTriangleMinMaxUVImpl(v1_, v2_, v0_, t12, t20, t01, W, H, bilinear, scissor, clamp, false, minU, minV, maxU, maxV);
	EdgeWalkTriangleMinMaxUVImpl(v2_, v0_, v1_, t20, t01, t12, W, H, bilinear, scissor, clamp, false, minU, minV, maxU, maxV);

	// Walk along edges in the anti-clockwise direction (if Y-axis points down)
	EdgeWalkTriangleMinMaxUVImpl(v0_, v1_, v2_, t01, t12, t20, W, H, bilinear, scissor, clamp, true, minU, minV, maxU, maxV);
	EdgeWalkTriangleMinMaxUVImpl(v1_, v2_, v0_, t12, t20, t01, W, H, bilinear, scissor, clamp, true, minU, minV, maxU, maxV);
	EdgeWalkTriangleMinMaxUVImpl(v2_, v0_, v1_, t20, t01, t12, W, H, bilinear, scissor, clamp, true, minU, minV, maxU, maxV);
}
...
void GSState::GetTriangleMinMaxUV(int W, int H, bool bilinear, int& minU, int& minV, int& maxU, int& maxV) const
{
	...
	if (s_n == s_n_debug)
	{
		if (save_points)
		{
			pointsHack.clear();
			pointsHackD.clear();
			origPts = fopen("C:\\Users\\tchan\\Desktop\\log_files\\origPts.txt", "w");
		}
	}
	...
	for (size_t tri_i = 0; tri_i < m_index.tail / 3; tri_i++)
	{
		...
		{
			int xi = static_cast<int>(m_vertex.buff[m_index.buff[3 * tri_i + vert_i]].XYZ.X);
			int yi = static_cast<int>(m_vertex.buff[m_index.buff[3 * tri_i + vert_i]].XYZ.Y);
			double x = static_cast<double>(xi - OFX) / 16.0;
			double y = static_cast<double>(yi - OFY) / 16.0;
			double U = static_cast<double>(m_vertex.buff[m_index.buff[3 * tri_i + vert_i]].U) / 16.0;
			double V = static_cast<double>(m_vertex.buff[m_index.buff[3 * tri_i + vert_i]].V) / 16.0;
			verts[vert_i] = Point(x, y, U, V);
			if (s_n == s_n_debug && save_points)
			{
				fprintf(origPts, "%f,%f,%f,%f\n", x, y, U, V);
			}
		}

		EdgeWalkTriangleMinMaxUV(verts[0], verts[1], verts[2], W, H, bilinear, m_context->SCISSOR, m_context->CLAMP, minU, minV, maxU, maxV);
	}
	...
	if (save_points)
	{
		...
		{
			{
				FILE* f = fopen("C:\\Users\\tchan\\Desktop\\log_files\\pointsHack.txt", "w");
				for (Point& p : pointsHack)
					p.y = std::abs(p.y);
				std::sort(pointsHack.begin(), pointsHack.end(), [](const Point& a, const Point& b) {
					if (a.y < b.y)
						return true;
					if (a.y > b.y)
						return false;
				...
				for (const Point& p : pointsHack)
				{
					fprintf(f, "%d,%d,%d,%d\n", (int)p.x, (int)p.y, (int)p.U, (int)p.V);
				}
				fflush(f);
				fclose(f);
				pointsHack.clear();
			...
			{
				FILE* f = fopen("C:\\Users\\tchan\\Desktop\\log_files\\pointsHack_D.txt", "w");
				std::sort(pointsHackD.begin(), pointsHackD.end(), [](const Point& a, const Point& b) {
					if (a.y < b.y)
						return true;
					if (a.y > b.y)
						return false;
...
void GSState::GetSpriteMinMaxUV(int W, int H, bool bilinear, int& minU, int& minV, int& maxU, int& maxV) const
{
	...
	for (size_t sprite_i = 0; sprite_i < m_index.tail / 2; sprite_i++)
	{
		...
		for (int vert_i = 0; vert_i < 2; vert_i++)
		{
			...
			double x = static_cast<double>(xi - OFX) / 16.0;
			double y = static_cast<double>(yi - OFY) / 16.0;
			double U = static_cast<double>(m_vertex.buff[m_index.buff[2 * sprite_i + vert_i]].U) / 16.0;
			double V = static_cast<double>(m_vertex.buff[m_index.buff[2 * sprite_i + vert_i]].V) / 16.0;
			verts[vert_i] = Point(x, y, U, V);
			if (s_n == s_n_debug && save_points)
			{
				fprintf(origPts, "%f,%f,%f,%f\n", x, y, U, V);
...
void GSState::FlushPrim()
{
	if (m_index.tail > 0)
	{
		...
		// Could put accurate caculation here
		...
		bool skip_draw = (m_context->TEST.ZTE && m_context->TEST.ZTST == ZTST_NEVER);
...
bool GSState::GetRegionRepeatMinMaxUV(int MSK, int FIX, int min, int max, int& min_out, int& max_out)
{
	...
	if (min < 0 && 0 <= max)
	{
		...
		bool modified = GSState::GetRegionRepeatMinMaxUV(MSK, FIX, min, -1, min_out_1, max_out_1) ||
						GSState::GetRegionRepeatMinMaxUV(MSK, FIX, 0, max, min_out_2, max_out_2);
		min_out = std::min(min_out_1, max_out_2);
		max_out = std::min(max_out_1, max_out_2);
	}
...
bool GSState::GetClampWrapMinMaxUV(int SIZE, int WM, int MIN, int MAX, int min, int max, int& min_out, int& max_out)
{
	...
	if (WM == CLAMP_REPEAT)
	{
		const int low_size_bits = SIZE - 1; // All bits below highest bit of SIZE.
		const int high_size_bits = ~low_size_bits; // All bits above (equal to the high bits of SIZE);
		// If we cross the SIZE boundary then we always get the largest/smallest possible wrapped value
		if ((min & high_size_bits) != (max & high_size_bits))
		{
			min_out = 0;
			max_out = SIZE - 1;
		}
		...
		{
			min_out = min & low_size_bits;
			max_out = max & high_size_bits;
		}
...
GSState::TextureMinMaxResult GSState::GetTextureMinMax(GIFRegTEX0 TEX0, GIFRegCLAMP CLAMP, bool linear, bool clamp_to_tsize)
{
	...
	else
	{
		...
		const int inclusive_x_req = 1;
		const int inclusive_y_req = 1;
