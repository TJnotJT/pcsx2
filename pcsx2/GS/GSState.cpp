	...
	if (skip == 0)
	{
		...
		if (m_nativeres)
		{
			...
			if constexpr (primclass == GS_POINT_CLASS || primclass == GS_LINE_CLASS)
			{
				...
				bbox = (bbox + GSVector4i(8)) & GSVector4i(~0xF);
			}
			else
			{
				// For triangles and sprites take the region strictly within the bounds omitting the bottom/right.
				bbox = ((bbox + GSVector4i(0xF, 0xF, -1, -1)) & GSVector4i(~0xF));
			}
		}

		if ((primclass == GS_LINE_CLASS || primclass == GS_TRIANGLE_CLASS) && PRIM->AA1)
		{
